...
use shanks_unified::ffi::types::{SeriesResult, SeriesPoint, AccelResult};
...
fn test_interval_ffi_integration() {
    ...
    let handle = lib.series_create("Ln2Series", "IntervalF64", "1.0", "{}")
        .expect("Failed to create IntervalF64 series");
    
    // Generate series
    let result_json = lib.series_generate(&handle, 10, false)
        .expect("Failed to generate series");
    
    // Parse it using the typed structs handling Interval parsing
    let parsed: SeriesResult = serde_json::from_str(&result_json)
        .expect("Failed to deserialize SeriesResult JSON");
    
    // Verify it contains interval points
    assert!(!parsed.sn.is_empty(), "Series result should not be empty");
    
    let first_point = &parsed.sn[0];
    match first_point {
    ...
    let accel_handle = lib.accel_create("Shanks Transformation", "IntervalF64", "{}")
        .expect("Failed to create accel handle");
        
    let accel_result = lib.accel_apply(&accel_handle, &handle, 10, 5, true)
        .expect("Failed to apply acceleration");
    
    let parsed_accel: AccelResult = serde_json::from_str(&accel_result)
        .expect("Failed to deserialize AccelResult JSON");
    assert!(!parsed_accel.values.is_empty(), "Accel results should not be empty");
    
    // Verify at least one valid computation exists and is an Interval
    let has_interval = parsed_accel.values.iter().filter_map(|v| v.as_ref()).any(|v| {
        matches!(v, SeriesPoint::Interval(_))
    });
    assert!(has_interval, "Expected at least one Interval result in acceleration values");
