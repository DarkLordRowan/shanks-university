...
interface SeriesComputedAnTableProps {
    points: DetailPoint[];
    useAbs: boolean;
}
...
export const SeriesComputedAnTable: React.FC<SeriesComputedAnTableProps> = ({ points, useAbs }) => {
    if (!points.length) return null;

    const headerErr = useAbs ? "|Sₙ − S|" : "sgn·|Sₙ − S|";
                ...
                <table className="min-w-full border-collapse text-[10px]">
                    <thead className="bg-surface/80">
                        <tr>
                            ...
                            <th className="border-b border-border px-2 py-1 text-left">Re(Sₙ)</th>
                            <th className="border-b border-border px-2 py-1 text-left">Im(Sₙ)</th>
                    ...
                    <tbody>
                        {points.slice(0, 200).map((p) => {
                            ...
                            if (err != null && Number.isFinite(err)) {
                                if (useAbs || !sign || sign === 0) errValue = err;
                                else errValue = err * sign;
                            }
