...
export const ErrorMatrixTable: React.FC<ErrorMatrixTableProps> = ({
    ...
    const pageSize = maxSteps && maxSteps > 0 ? maxSteps : nList.length || 1;

    const totalPages = Math.max(1, Math.ceil(nList.length / pageSize || 1));
    ...
    const nListShown = maxSteps && maxSteps > 0 ? nList.slice(startIndex, endIndex) : nList;
        ...
        <div className={`space-y-2 ${className ?? ""}`}>
            <div className="flex items-center justify-between text-xs text-textDim">
                ...
                <div className="flex items-center gap-3">
                    <div>
                        Алгоритмы: {sortedAlgoList.length} · Шаги n: {nListShown.length} из{" "}
                        {nList.length} · Элементов с ошибкой (с n): {totalErrorItems} · Всего
                    ...
                    {maxSteps && maxSteps > 0 && nList.length > maxSteps && (
                        <div className="flex items-center gap-1 text-[10px]">
                            ...
                            <span className="text-textDim/60">
                                шаги {startIndex + 1}–{Math.min(endIndex, nList.length)}
                            </span>
                            <button
                                type="button"
                                className="rounded border border-border bg-surface px-1 py-[1px] disabled:opacity-40 hover:bg-panel"
                        ...
                        <tr>
                            ...
                            {nListShown.map((n) => (
                                <th
                                    ...
                                    title={`Шаг n = ${n}`}
                                >
                                    <div className="w-[32px] text-center text-[10px]">n={n}</div>
                                </th>
                            ))}
                        </tr>
                    </thead>
                                ...
                                <tr key={algo.key}>
                                    ...
                                    <th
                                        ...
                                        title={(() => {
                                            ...
                                            lines.push(`m = ${algo.m != null ? algo.m : "∅"}`);
                                            if (
                                                algo.algorithmArgs &&
                                                Object.keys(algo.algorithmArgs).length > 0
                                            ) {
                                            ...
                                            lines.push("");
                                            lines.push(`Всего: ${total}, OK: ${ok}, Err: ${err}`);
                                            if (okPct != null) {
                                                lines.push(
                                                    `% OK ≈ ${okPct.toFixed(
                                                        1
                                            ...
                                            if (precision) {
                                                lines.push(`Фильтр precision = ${precision}`);
                                            }
                                            return lines.join("\n");
                                        })()}
                                    >
