...
export function useProfilingMatrix(
    ...
    const [matrix, setMatrix] = useState<ProfilingMatrix | null>(null);
    ...
    useEffect(() => {
        ...
        for (const s of seriesListRaw) seriesMap.set(s.id, s);

        const accelMap = new Map<string, Accel>();
        for (const a of accelListRaw) accelMap.set(a.id, a);
        ...
        function processChunk(startIndex: number) {
            ...
            for (let i = startIndex; i < endIndex; i++) {
                ...
                const analysis = analyzeSeriesAccelProfiling(sa);
