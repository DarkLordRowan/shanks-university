...
import {
    ...
    intervalsFromDomain,
    latexToNumber,
} from "@/shared/lib/domain/domain-intervals";
import type { DomainSpec, SeriesNode } from "@/entities/experiment/model/seriesData";
...
export function SelectXs({ series, value, onChange, disabled }: Props) {
    ...
    const intervals = useMemo(() => (domain ? intervalsFromDomain(domain) : []), [domain]);

    const quick = useMemo(() => (domain ? characteristicPoints(domain) : []), [domain]);
        ...
        return (
            <div className="rounded-xl2 border border-border bg-panel p-3 text-sm text-white shadow-panel">
                <div className="text-xs text-textDim">
                    Выбор нескольких x. Сначала выберите ряд.
                </div>
            </div>
        );
    }
    ...
    if (collapsed) {
        const preview = value
            .slice(0, 5)
            .map((v) => (Number.isInteger(v) ? v.toString() : v.toPrecision(8)));
        return (
    ...
    return (
        <div className="rounded-xl2 border border-border bg-panel p-3 text-sm text-white shadow-panel">
            ...
            <div className="flex flex-col gap-2">
                <label
                    htmlFor={`${id}-xs`}
                    className="flex items-center gap-2 text-[11px] text-textDim"
                >
                    <span>Добавьте</span>
            ...
            <div className="mt-3 flex items-center gap-2 text-[11px]">
                {draftStatus === "valid" && (
                    <span className="text-emerald-500">Готово к добавлению.</span>
                )}
                {draftStatus === "out" && <span className="text-red-500">Вне области домена.</span>}
                {draftStatus === "dup" && (
                    <span className="text-textDim">Такое значение уже добавлено.</span>
                )}
                {draftStatus === "bad" && (
                    <span className="text-red-500">Не удалось распарсить.</span>
                )}
                <button
                    type="button"
                    onClick={clearAll}
