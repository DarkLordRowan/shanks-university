...
interface SeriesComputedDiffTableProps {
    points: DetailPoint[];
    useAbs: boolean;
}
...
export const SeriesComputedDiffTable: React.FC<SeriesComputedDiffTableProps> = ({
    points,
    useAbs,
}) => {
    const hasDiff = points.some((p) => p.diffNorm != null);

    const headerNorm = useAbs ? "|Sₙ − Sₙ₋₁|" : "sgn·|Sₙ − Sₙ₋₁|";

    return (
        <div className="mt-4 border-t border-border/60 pt-3">
            <div className="mb-2 text-[11px] font-semibold text-textDim">Разности Sₙ − Sₙ₋₁</div>
            ...
            {hasDiff ? (
                ...
                <div className="text-[11px] text-textDim/70">
                    Недостаточно данных для вычисления разностей Sₙ − Sₙ₋₁.
                </div>
            )}
        </div>
    );
