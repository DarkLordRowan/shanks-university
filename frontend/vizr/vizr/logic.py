...
from PyQt6.QtGui import QColor  # needed for color generation logic, though it's UI-related

# we import the data types
from .data_loader import SeriesRecord, AccelRecord, FilteredMethod
    ...
    def __init__(self, data: List[Tuple[SeriesRecord, List[AccelRecord]]]):
        ...
        for series, accels in data:
            ...
            for k, v in series.arguments.items():
                if k not in self.series_args: self.series_args[k] = set()
            ...
            for accel in accels:
                ...
                for k, v in accel.accel_info.additional_args.items():
                    if k not in self.accel_args: self.accel_args[k] = set()
...
def prepare_viz_data(
    ...
    for i, (series, accels) in enumerate(data):
        ...
        viz_data.legend_items.append(LegendItemData(
            text=series_label,
            color=series_color
        ))
        ...
        if options.show_partial_sums:
            if options.show_real:
                ...
                viz_data.convergence_curves.append(CurveData(
                    x=n_vals, y=real_vals,
                ...
                viz_data.error_curves.append(CurveData(
                    x=n_vals, y=series_dev,
            ...
            if options.show_imaginary:
                ...
                if options.force_show_imaginary or not is_zero_imag:
                    imag_vals = vectorized_approx_f64(series.val_imag_m, series.val_imag_e)
                    viz_data.convergence_curves.append(CurveData(
                        x=n_vals, y=imag_vals,
        ...
        if options.show_limits and len(n_vals) > 0:
            ...
            if options.show_real:
                ...
                viz_data.convergence_curves.append(CurveData(
                    x=np.array([min_x, max_x]), y=np.array([real_limit, real_limit]),
            ...
            if options.show_imaginary:
                ...
                if options.force_show_imaginary or not is_zero_imag:
                    viz_data.convergence_curves.append(CurveData(
                        x=np.array([min_x, max_x]), y=np.array([imag_limit, imag_limit]),
        ...
        for accel in accels:
            ...
            if options.show_real:
                ...
                accel_dev_real = vectorized_approx_f64(d_m, d_e) # keep raw for table/perf
                ...
                batch_real_segs.append((curr_off_real, curr_off_real + seg_len, accel_tooltip))
                ...
                if rows_added < max_table_rows:
                     # stats
                    valid_real_indices = np.where(~np.isnan(accel_dev_real) & current_mask)[0]
                    ...
                    def make_val_list(vals, ns):
                        return [f"n={int(n)}: {v:.6e}" for v, n in zip(vals, ns) if not np.isnan(v)]
                    
                    viz_data.table_rows.append(TableRowData(
                        ...
                        series_values=[
                            ...
                            (f" + {series.val_imag_m[k]*(10.0**series.val_imag_e[k]):.6e}j" if series.val_imag_m[k] != 0 else "")
                            for k in range(len(series.n))
                        ],
                        accel_values=make_val_list(accel_real, current_n),
                        deviations=make_val_list(accel_dev_real, current_n), # use real deviation for text
                        errors=[f"n={e.n}: {e.message}" for e in accel.errors],
                        events=[f"n={e.n}: {e.name} - {e.description}" for e in accel.events],
            ...
            if options.show_imaginary:
                ...
                if options.force_show_imaginary or not is_zero_imag:
                    ...
                    seg_len = len(current_n)
                    batch_imag_segs.append((curr_off_imag, curr_off_imag + seg_len, f"{accel_tooltip} (Imag)"))
            ...
            if options.show_filters and accel.filtered:
                for method in accel.filtered.methods:
                    ...
                    if options.show_real:
                        ...
                        batch_filt_segs.append((curr_off_filt, curr_off_filt + f_len, filter_tooltip))
                    ...
                    if options.show_imaginary:
                        ...
                        if options.force_show_imaginary or not is_zero_imag:
                            ...
                            batch_filt_imag_segs.append((curr_off_filt_imag, curr_off_filt_imag + f_len, f"{filter_tooltip} (Imag)"))
        ...
        if batch_real_n:
            ...
            # convergence
            viz_data.convergence_curves.append(CurveData(
                x=all_n, y=np.concatenate(batch_real_y),
                pen=pg.mkPen(series_color, width=1, style=Qt.PenStyle.DashLine, cosmetic=True),
            ...
            # error
            viz_data.error_curves.append(CurveData(
                x=all_n, y=np.concatenate(batch_real_err),
                pen=pg.mkPen(series_color, width=1, style=Qt.PenStyle.DashLine, cosmetic=True),
                name=f"{series_label} (accels)",
                segments=batch_real_segs
            ))
        ...
        if batch_imag_n:
            viz_data.convergence_curves.append(CurveData(
                x=np.concatenate(batch_imag_n), y=np.concatenate(batch_imag_y),
                pen=pg.mkPen(COLOR_ACCEL_IMAG, width=1, style=Qt.PenStyle.DashLine, cosmetic=True),
        ...
        if batch_filt_n:
             viz_data.convergence_curves.append(CurveData(
                x=np.concatenate(batch_filt_n), y=np.concatenate(batch_filt_y),
                pen=pg.mkPen(COLOR_FILTER_REAL, width=2, style=Qt.PenStyle.DotLine, cosmetic=True),
        ...
        if batch_filt_err_n:
            viz_data.error_curves.append(CurveData(
                x=np.concatenate(batch_filt_err_n), y=np.concatenate(batch_filt_err_y),
                pen=pg.mkPen(COLOR_FILTER_REAL, width=2, style=Qt.PenStyle.DotLine, cosmetic=True),
        ...
        if batch_filt_imag_n:
             viz_data.convergence_curves.append(CurveData(
                x=np.concatenate(batch_filt_imag_n), y=np.concatenate(batch_filt_imag_y),
                pen=pg.mkPen(COLOR_FILTER_IMAG, width=2, style=Qt.PenStyle.DotLine, cosmetic=True),
    ...
    if perf_x:
        viz_data.performance_data = ScatterData(x=perf_x, y=perf_y, brushes=perf_brushes, names=perf_names)
