...
public:
	...
	*/
	explicit e_x_series() : series_base<T, K>("e_x_series") {};
...
template<AcceptedLike T, UnsignedIntLike K>
SeriesResult<T> e_x_series<T, K>::generateSeries(
    const T& x , 
	const K vecSize, 
	...
	std::vector<T> vecAn(vecSize, convertWithPrec<T>(0.0, series_base<T, K>::precision)); vecAn[0] = static_cast<T>(std::numbers::pi * 0,5);
	std::vector<T> vecSn(vecSize, convertWithPrec<T>(0.0, series_base<T, K>::precision)); vecSn[0] = static_cast<T>(std::numbers::pi * 0,5);
