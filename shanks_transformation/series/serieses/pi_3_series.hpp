...
public:
	...
	*/
	explicit pi_3_series() : series_base<T, K>("pi_3_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x * static_cast<T>(std::numbers::pi) / static_cast<T>(3);
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> pi_3_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(0); j < vecSize; ++j){
		vecAn[j] += x / static_cast<T>((j+1)*fma(2,j,1)*fma(4,j,1));
