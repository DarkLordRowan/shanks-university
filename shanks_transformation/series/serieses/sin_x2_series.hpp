...
public:
	...
	*/
	explicit sin_x2_series() : series_base<T, K>("sin_x2_series") {};
...
template<AcceptedLike T, UnsignedIntLike K>
series_result<T> sin_x2_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(1); j < vecSize; ++j){
		vecAn[j] += static_cast<T>(-1) * vecAn[j-static_cast<K>(1)] * x_4 / static_cast<T>(fma(2,j,1));
