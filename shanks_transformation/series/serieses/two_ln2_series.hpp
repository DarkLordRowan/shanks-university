...
public:
	...
	*/
	explicit two_ln2_series() : series_base<T, K>("two_ln2_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x * log(static_cast<T>(4));
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> two_ln2_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(1); j < vecSize; ++j){
		vecAn[j] += static_cast<T>(12 * (j + 1) * (j + 1) - 1) * x / static_cast<T>((j+1)*(4*(j+1*(j+1)-1))*(4*(j+1*(j+1)-1)));
