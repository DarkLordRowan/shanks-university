...
protected:
    ...
    inline bool domain_checker(const SeriesConfig<T,K>& config) const{ 
		...
		if constexpr ( std::is_floating_point<T>::value || std::is_same<T, float_precision>::value)
			return config.x < static_cast<T>(0) || !isfinite(config.x); 

		if constexpr ( std::is_same<T, complex_precision<float_precision>>::value )
			return config.x.real() < static_cast<float_precision>(0) || !isfinite(config.x); 
	 ...
	 * @return constexpr T 
	 */
	T calculate_sum() const  { return cos(sqrt(this->x)); }
