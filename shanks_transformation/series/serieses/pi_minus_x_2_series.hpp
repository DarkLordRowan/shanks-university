...
template<AcceptedLike T, UnsignedIntLike K>
series_result<T> pi_minus_x_2_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
...
class pi_minus_x_2_series<complex_precision<T>, K> final : public series_base<complex_precision<T>, K>
{
public:
	...
	* @param x The argument for function series
	*/
	explicit pi_minus_x_2_series() : series_base<complex_precision<T>, K>("pi_minus_x_2_series") {};
	...
	inline constexpr bool check_domain(const complex_precision<T>& x){
    	...
    	return !isfinite(x.real()) || !isfinite(x.imag()) || x.real() <= static_cast<T>(0) || x.real() >= static_cast<T>(std::numbers::pi);

	}

	inline constexpr complex_precision<T> calculate_sum(const complex_precision<T>& x){

		return (complex_precision<T>(std::numbers::pi) - x) * complex_precision<T>(0.5);
	}

};

template<FloatLike T, UnsignedIntLike K>
series_result<complex_precision<T>> pi_minus_x_2_series<complex_precision<T>, K>::generate_series(
    const complex_precision<T>& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(0); j < vecSize; ++j){
		vecAn[j] += sin(Complex(j+1)*x) / Complex(j+1);
		vecSn[j] += vecSn[j == static_cast<K>(0) ? j : j-static_cast<K>(1)] + vecAn[j];
	}
