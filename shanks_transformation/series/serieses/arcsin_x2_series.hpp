...
constexpr T arcsin_x2_series<T, K>::operator()(K n) const
{
    ...
    return (this->fact(a - static_cast <K>(1)) * pow(this->x, static_cast<T>(static_cast <K>(2) * a))) / static_cast<T>(pow(static_cast <K>(4), n) * pow(this->fact(n), static_cast <K>(2)) * a); // (84.1) [Rows.pdf]
}
