...
template<AcceptedLike T, UnsignedIntLike K>
SeriesResult<T> exp_m_cos_x_sinsin_x_series<T, K>::generateSeries(
    const T& x , 
	const K vecSize, 
	...
	std::vector<T> vecAn(vecSize, convertWithPrec<T>(0.0, series_base<T, K>::precision)); vecAn[0] = static_cast<T>(0);
	std::vector<T> vecSn(vecSize, convertWithPrec<T>(0.0, series_base<T, K>::precision)); vecSn[0] = static_cast<T>(0);
