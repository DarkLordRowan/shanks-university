...
public:
	...
	*/
	explicit x_div_1minx2_series() : series_base<T, K>("x_div_1minx2_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x / sqrt(static_cast<T>(1) - x*x);
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> x_div_1minx2_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(1); j < vecSize; ++j){
		vecAn[j] += vecAn[j-static_cast<K>(1)] * x * x * static_cast<T>(fma(2,j-1,1)) / static_cast<T>(2 * j);
...
class x_div_1minx2_series<complex_precision<T>,K> final : public series_base<complex_precision<T>,K>
{
public:
	...
	* @param x The argument for function series
	*/
	explicit x_div_1minx2_series() : series_base<complex_precision<T>, K>("x_div_1minx2_series") {};
	...
	inline constexpr complex_precision<T> calculate_sum(const complex_precision<T>& x){
		...
		return x / sqrt(complex_precision<T>(1) - x*x);
	}

};

template<FloatLike T, UnsignedIntLike K>
series_result<complex_precision<T>> x_div_1minx2_series<complex_precision<T>, K>::generate_series(
    const complex_precision<T>& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(1); j < vecSize; ++j){
		vecAn[j] += vecAn[j-static_cast<K>(1)] * x * x * Complex(fma(2,j-1,1)) / Complex(2 * j);
		vecSn[j] += vecSn[j-static_cast<K>(1)] + vecAn[j];
	}
