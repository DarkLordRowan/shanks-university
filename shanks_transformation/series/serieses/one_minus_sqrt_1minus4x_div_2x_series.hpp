	...
	*/
	...
	inline constexpr bool check_domain(const T& x){
		...
		if constexpr (isComplexLike<T>::value){
    		return !isfinite(x.real()) || !isfinite(x.imag()) || abs(x) > abs(static_cast<T>(0.25)) || x == static_cast<T>(0);
        } else {
		    return !isfinite(x) || abs(x) > static_cast<T>(0.25) || x == static_cast<T>(0);;
        }
	}
...
template<AcceptedLike T, UnsignedIntLike K>
series_result<T> one_minus_sqrt_1minus4x_div_2x_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	if(check_domain(x)){
		series_base<T, K>::throw_domain_error("x is not finite or |x|>0.25 or x = 0");
	}
	...
	for(K j = static_cast<K>(2); j < vecSize; ++j){
		vecAn[j] += vecAn[j-static_cast<K>(1)] * x * static_cast<T>(2*fma(2,j-1,1)) / static_cast<T>(j+1);
