...
template<AcceptedLike T, UnsignedIntLike K>
SeriesResult<T> x_min_sqrt_x_series<T, K>::generateSeries(
    const T& x , 
	const K vecSize, 
	...
	std::vector<T> vecSn(vecSize, convertWithPrec<T>(0.0, series_base<T, K>::precision)); vecSn[0] = x - static_cast<T>(1);
