...
template <std::floating_point T, std::unsigned_integral K>
one_div_sqrt2_sin_xdivsqrt2_series<T, K>::one_div_sqrt2_sin_xdivsqrt2_series(T x) : series_base<T, K>(x, (static_cast<T>(1) / static_cast<T>(std::sqrt(2))) * static_cast<T>(std::sin(x / static_cast<T>(std::sqrt(2)))))
{
...
{
    const K tmp = static_cast<K>(std::fma(2, n, 1));
    return static_cast<T>(std::pow(-1, n / 2)) * static_cast<T>(std::cyl_bessel_j(tmp, this->x)); // (96.1) [Rows.pdf]
}
