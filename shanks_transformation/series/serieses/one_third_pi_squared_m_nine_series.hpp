...
public:
	...
	*/
	...
	inline constexpr T calculateSum(const T& x){
		return x * (static_cast<T>(std::numbers::pi) * static_cast<T>(std::numbers::pi) - static_cast<T>(9)) / static_cast<T>(3);
	}

};

template<AcceptedLike T, UnsignedIntLike K>
SeriesResult<T> one_third_pi_squared_m_nine_series<T, K>::generateSeries(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(0); j < vecSize; ++j){
		vecAn[j] += x / static_cast<T>((j+1)*(j+1)*(j+2)*(j+2));
