...
public:
	...
	*/
	explicit Ja_x_series() : series_base<T, K>("Ja_x_series") {};
	...
	inline constexpr T calculateSum(const T& x){
		...
		return static_cast<T>(1) / (static_cast<T>(1) - x);
	}

};

template<AcceptedLike T, UnsignedIntLike K>
SeriesResult<T> Ja_x_series<T, K>::generateSeries(
    const T& x , 
	const K vecSize, 
    ...
    series_base<T,K>::series_name = "Ja_x_series";
