...
{
    return this->minus_one_raised_to_power_n(n) * pow(this->x * static_cast <T>(0.5), static_cast<T>(2 * n) + this->a) / (static_cast<T>(this->fact(n)) * tgamma(this->a + static_cast<T>(n + 1))); // (95.1) [Rows.pdf]
}
