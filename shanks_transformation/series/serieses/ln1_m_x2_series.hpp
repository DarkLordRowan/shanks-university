...
class ln1_m_x2_series final : public TermCalculatorBase<T, K>
{
protected:
	 ...
	 * @return constexpr T 
	 */
	T calculate_sum() const  { return log(static_cast<T>(1) - this->x * this->x); }
...
public:
	...
	*/
	 ...
	 * @param config 
	 */
	ln1_m_x2_series(const SeriesConfig<T,K>& config);
};

template <Accepted T, std::unsigned_integral K>
ln1_m_x2_series<T, K>::ln1_m_x2_series(const SeriesConfig<T,K>& config) {
...
template <Accepted T, std::unsigned_integral K>
constexpr T ln1_m_x2_series<T, K>::calculateTerm(K n) const {
