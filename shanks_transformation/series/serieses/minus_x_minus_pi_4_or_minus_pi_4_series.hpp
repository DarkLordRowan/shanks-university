...
template<AcceptedLike T, UnsignedIntLike K>
SeriesResult<T> minus_x_minus_pi_4_or_minus_pi_4_series<T, K>::generateSeries(
    const T& x , 
	const K vecSize, 
	...
	std::vector<T> vecAn(vecSize, convertWithPrec<T>(0.0, series_base<T, K>::precision)); vecAn[0] = static_cast<T>(std::numbers::pi * 0.25);
	std::vector<T> vecSn(vecSize, convertWithPrec<T>(0.0, series_base<T, K>::precision)); vecSn[0] = static_cast<T>(std::numbers::pi * 0.25);
