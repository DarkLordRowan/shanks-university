...
template <Accepted T, std::unsigned_integral K>
half_multi_ln_1div2multi1minuscosx_series<T, K>::half_multi_ln_1div2multi1minuscosx_series(T x) : series_base<T, K>(x, static_cast<T>(0.5) * log(static_cast<T>(0.5) / (static_cast<T>(1) -  cos(x))))
{
    ...
    if (x <= static_cast<T>(0) || x >= static_cast<T>(2.0 * std::numbers::pi) || !isfinite(x) || cos(x) == static_cast<T>(1)) {
