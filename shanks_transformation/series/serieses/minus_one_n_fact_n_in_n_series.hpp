...
template<AcceptedLike T, UnsignedIntLike K>
SeriesResult<T> minus_one_n_fact_n_in_n_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(0); j < vecSize; ++j){
		vecAn[j] += minus_one_raised_to_power_n<T, K>(j + 1) * static_cast<T>(fact<K>(j+1)) * x / static_cast<T>(pow(j+1, j+1));
