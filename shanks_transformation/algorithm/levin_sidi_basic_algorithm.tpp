...
levin_sidi_basic_algorithm<T, K, series_templ>::levin_sidi_basic_algorithm(const series_templ &series, T beta_)
    : series_acceleration<T, K, series_templ>(series), beta(beta_) {
}

template<typename T, typename K, typename series_templ>
template<class remainderType>
T levin_sidi_basic_algorithm<T, K, series_templ>::calculate(K k, int n, remainderType remainder_func) const {
    if (n < 0)
...
template<typename T, typename K, typename series_templ>
template<class remainderType>
T levin_sidi_basic_algorithm<T, K, series_templ>::calculate_recursively(K k, int n, remainderType remainder_func) const {
    if (n < 0)
        throw std::domain_error("negative integer in input");
