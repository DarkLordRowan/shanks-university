...
levin_basic_algorithm<T, K, series_templ>::levin_basic_algorithm(const series_templ &series)
    : series_acceleration<T, K, series_templ>(series) {
}

template<typename T, typename K, typename series_templ>
T levin_basic_algorithm<T, K, series_templ>::operator()(const K n, const int order) const {
    if (n < 0)
