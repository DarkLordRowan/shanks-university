...
template<typename T, typename K, typename series_templ>
shanks_transform<T, K,
    series_templ>::shanks_transform(const series_templ &series) : series_acceleration<T, K, series_templ>(series) {
}
...
T shanks_transform<T, K, series_templ>::operator()(const K n, const int order) const {
    ...
    if (order == 0) [[unlikely]]
            /*it is convenient to assume that transformation of order 0 is no transformation at all*/
    ...
    if (order == 1) [[unlikely]] {
        ...
        const T result = std::fma(a_n * a_n_plus_1,
                                  (a_n + a_n_plus_1) / (
                                      std::fma(a_n, a_n, tmp) - std::fma(a_n_plus_1, a_n_plus_1, tmp)),
                                  this->series->S_n(n));
    ...
    // if we got to this branch then we know that n >= order - see previous branches  int -> K
    {
        ...
        //formula[6]
        T_n[i] = std::fma(a_n * a_n_plus_1,
                          (a_n + a_n_plus_1) / (std::fma(a_n, a_n, tmp) - std::fma(a_n_plus_1, a_n_plus_1, tmp)),
                          this->series->S_n(i));
    }
...
template<typename T, typename K, typename series_templ>
shanks_transform_alternating<T, K,
    series_templ>::shanks_transform_alternating(const series_templ &series) : series_acceleration<T, K, series_templ>(
    series) {
}
