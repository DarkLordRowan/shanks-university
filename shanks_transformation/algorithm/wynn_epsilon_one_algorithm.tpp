...
wynn_epsilon_one_algorithm<T, K, series_templ>::wynn_epsilon_one_algorithm(const series_templ &series)
    : series_acceleration<T, K, series_templ>(series) {

}

template<typename T, typename K, typename series_templ>
T wynn_epsilon_one_algorithm<T, K, series_templ>::operator()(const K n, const int order) const {
    int m = 2 * order;
    if (n <= 0)
