...
T lubkin_w_algorithm<T, K, series_templ>::calculate(K n, const int order, T S_n, const K j) const {
    /*
    * j - to fix n
...
template<typename T, typename K, typename series_templ>
lubkin_w_algorithm<T, K, series_templ>::lubkin_w_algorithm(const series_templ &series)
    : series_acceleration<T, K, series_templ>(series) {
}

template<typename T, typename K, typename series_templ>
T lubkin_w_algorithm<T, K, series_templ>::operator()(const K n, const int order) const {
    if (order < 0)
