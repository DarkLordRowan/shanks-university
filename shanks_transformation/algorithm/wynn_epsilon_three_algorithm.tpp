...
wynn_epsilon_three_algorithm<T, K, series_templ>::
wynn_epsilon_three_algorithm(const series_templ &series, T epsilon_threshold_)
    : series_acceleration<T, K, series_templ>(series), epsilon_threshold(epsilon_threshold_) {
}

template<typename T, typename K, typename series_templ>
T wynn_epsilon_three_algorithm<T, K, series_templ>::operator()(const K n, const int order) const {
    if (n < 0)
