...
namespace series{
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	exp_series<T, K>::exp_series(T x) : series_base<T, K>(x, exp(x)), reccurent_series_base<T, K>(static_cast<T>(1))
	{
	...
	T exp_series<T, K>::access_row(K n)
	{
		...
		for (auto i = old_size; i <= static_cast<typename std::vector<T>::size_type>(n); ++i)
			this->series_vector.push_back(this->series_vector[i - 1] * this->x / static_cast<T>(i));
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	cos_series<T, K>::cos_series(T x) : series_base<T, K>(x, cos(x)), reccurent_series_base<T, K>(std::vector<T>{static_cast<T>(1), (static_cast<T>(-1) * x * x) / static_cast<T>(2)})
	{
	...
	T cos_series<T, K>::access_row(K n)
	{
		...
		T a = static_cast<T>(-1) * this->x * this->x;

		for (auto i = old_size; i <= static_cast<typename std::vector<T>::size_type>(n); ++i)
			series_vec.push_back(series_vec[i - 1] * (a / static_cast<T>(i * fma(4, i, -2))));
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	sin_series<T, K>::sin_series(T x) : series_base<T, K>(x, sin(x)), reccurent_series_base<T, K>(x) {
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	cosh_series<T, K>::cosh_series(T x) : series_base<T, K>(x, cosh(x)), reccurent_series_base<T, K>(std::vector<T>{static_cast<T>(1), (x * x) / static_cast<T>(2)})
	{
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	sinh_series<T, K>::sinh_series(T x) : series_base<T, K>(x, sinh(x)), reccurent_series_base<T, K>(x)
	{
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	bin_series<T, K>::bin_series(T x, T alpha) : series_base<T, K>(x, static_cast<T>(pow(static_cast<T>(1) + x, alpha))), alpha(alpha)
	{
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	ln1mx_series<T, K>::ln1mx_series(T x) : 
	series_base<T, K>(
		x,
		 static_cast<T>(-1)*log(static_cast<T>(1) - x)
		)
	{
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	mean_sinh_sin_series<T, K>::mean_sinh_sin_series(T x) : 
	series_base<T, K>(
		x, 
		static_cast<T>(0.5) * (sinh(x) + sin(x))
	), reccurent_series_base<T, K>(x)
	{
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	xmb_Jb_two_series<T, K>::xmb_Jb_two_series(T x, K b) : series_base<T, K>(
		x, 
		static_cast<T>(1) / pow(x, static_cast<T>(b)) * [&]{
			if constexpr (std::is_same<T, float_precision>::value){
				return static_cast<T>(0); 
			} else {
				return std::cyl_bessel_j(b, 2 * x);
			}
		}() //костыль, т.к. я не выкупаю как это сделать нормально
	), mu(b) 
	{
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	half_asin_two_x_series<T, K>::half_asin_two_x_series(T x) : series_base<T, K>(x, static_cast<T>(0.5) * static_cast<T>(asin(static_cast<T>(2) * x)))
	{
		this->series_name = "0.5*asin(2x)";
		// Сходится при |x| ≤ 0.5 (абсолютно при |x| < 0.5, условно при |x| = 0.5)

		if (abs(x) > static_cast<T>(0.5) || !isfinite(x)) {
	...
	constexpr T half_asin_two_x_series<T, K>::operator()(K n) const
	{
		...
		const K a = 2 * n;
		return static_cast<T>(this->fact(a)) * pow(this->x, static_cast<T>(a + 1)) / (_fact_n * _fact_n * static_cast<T>((a + 1))); // (12.1) [Rows.pdf]
	}
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	erf_series<T, K>::erf_series(T x) : 
	series_base<T, K>(
		x, 
		static_cast<T>(0.5) * sqrt(static_cast<T>(std::numbers::pi)) * erf(x)
	), reccurent_series_base<T, K>(x)
	{
	...
	T erf_series<T, K>::access_row(K n)
	{
		...
		series_vec.reserve(n);
		T a;
		T b = static_cast<T>(-1) * this->x * this->x;
	...
	*/
	...
	series_base<T, K>(
		...
		m(m), 
		reccurent_series_base<T, K>(static_cast<T>(this->fact(m))
	)
	{
		...
		if (abs(this->x) >= static_cast<T>(1) || !isfinite(x)) {
	...
	*/
	...
	{
		const K a = n + 1;
		return series_base<T, K>::minus_one_raised_to_power_n(a) * cos(static_cast<T>(a) * this->x) / static_cast<T>(a * a); // (18.2) [Rows.pdf]
	}
	...
	*/
	...
	{
		return static_cast<T>(n ? this->x / static_cast<T>(fma(n, n, n)) : static_cast<T>(0)); // (21.5) [Rows.pdf]
	}
	...
	*/
	...
	{
		return static_cast<T>(n ? series_base<T, K>::minus_one_raised_to_power_n(n) * this->x / static_cast<T>(n * (n + static_cast<K>(2))) : static_cast<T>(0)); // (22.2) [Rows.pdf]
	}
	...
	*/
	...
	{
		return static_cast<T>(n ? this->x / static_cast<T>(n * n * (n + static_cast<K>(1))) : static_cast<T>(0)); // (25.3) [Rows.pdf]
	}
	...
	*/
	...
	{
		return static_cast<T>(n ? series_base<T, K>::minus_one_raised_to_power_n(n) * this-> x / static_cast<T>(n * (n + static_cast<K>(1)) * fma(2, n, 1)) : static_cast<T>(0)); // (26.2) [Rows.pdf]
	}
	...
	*/
	...
	constexpr T one_third_pi_squared_m_nine_series<T, K>::operator()(K n) const
	{
		...
		return static_cast<T>(n ? this->x / static_cast<T>(a * a) : static_cast<T>(0)); // (29.2) [Rows.pdf]
	}
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	four_ln2_m_3_series<T, K>::four_ln2_m_3_series(T x) : 
	series_base<T, K>(
		x, 
		x * fma(static_cast<T>(4), log(static_cast<T>(2)), static_cast<T>(-3))
	)
	{
	...
	{
		const K a = n * (n + static_cast<K>(1));
		return static_cast<T>(n ? series_base<T, K>::minus_one_raised_to_power_n(n) * this->x / static_cast<T>(a * a) : static_cast<T>(0)); // (30.2) [Rows.pdf]
	}
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	pi_four_minus_ln2_halfed_series<T, K>::pi_four_minus_ln2_halfed_series(T x) : series_base<T, K>(x, x * (static_cast<T>(std::numbers::pi) / static_cast<T>(4) - log(static_cast<T>(2)) / static_cast<T>(2)))
	{
	...
	{
		return static_cast<T>(n ? this->x * series_base<T, K>::minus_one_raised_to_power_n(static_cast<K>(std::trunc(n / 2))) / static_cast<T>(n) : static_cast<T>(0)); // (32.2) [Rows.pdf]
	}
	...
	*/
	...
	{
		return this->x * series_base<T, K>::minus_one_raised_to_power_n(static_cast<K>(std::trunc(n / 3))) / static_cast<T>(fma(2, n, 1)); // (33.2) [Rows.pdf]
	}
	...
	*/
	...
	{
		return static_cast<T>(n ? this->x * static_cast<T>(fma(2, n * n, -1)) / static_cast<T>(fma(4, pow(n, 4), 1)) : static_cast<T>(0)); // (36.2) [Rows.pdf]
	}
	...
	*/
	...
	{
		return static_cast<T>(n ? this->x * series_base<T, K>::minus_one_raised_to_power_n(n) / static_cast<T>(pow(n, n)) : static_cast<T>(0)); // (37.2) [Rows.pdf]
	}
	...
	*/
	...
	{
		return static_cast<T>(n ? this->x * static_cast<T>(this->fact(n)) * series_base<T, K>::minus_one_raised_to_power_n(n) / static_cast<T>(pow(n, n)) : static_cast<T>(0)); // (38.2) [Rows.pdf]
	}
	...
	*/
	...
	series_base<T, K>(
		x, 
		log((static_cast<T>(1) + x) / (static_cast<T>(1) - x)) / static_cast<T>(2)
	)
	{
	...
	*/
	...
	series_base<T, K>(x, 
		static_cast<T>(2) * asin(x / static_cast<T>(2)) * asin(x / static_cast<T>(2))
	)
	{
	...
	{
		const K a = static_cast<K>(fma(2, n, 1));
		// return (this->binomial_coefficient(2 * n, n) * this->binomial_coefficient(2 * n, n) * pow(this->x, 2 * a)) / (a * a * pow(2, std::fma(3, a, -2)));
		return pow(this->x, static_cast<T>(a - 1)) *  static_cast<T>(this->fact(n)) * static_cast<T>(this->fact(n)) / (static_cast<T>(2 * (n + 1)) * static_cast<T>(this->fact(a)));
	}
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	pi_cubed_32_series<T, K>::pi_cubed_32_series(T x) : 
	series_base<T, K>(
		x, 
		x * pow(static_cast<T>(std::numbers::pi), static_cast<T>(3)) / static_cast<T>(32)
	)
	{
	...
	*/
	...
	{
		const T n_temp = static_cast<T>(n + 1);
		return this->x / (n_temp * fma(static_cast<T>(36), n_temp * n_temp, static_cast<T>(-1)));  // (43.2) [Rows.pdf]
	}
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	two_ln2_series<T, K>::two_ln2_series(T x) : series_base<T, K>(x, x * log(static_cast<T>(4)))
	{
	...
	*/
	...
	)
	{
		...
		if (x <= static_cast<T>(0) || x >= static_cast<T>(2 * std::numbers::pi) || !isfinite(x) || cos(x) == static_cast<T>(1)) {
	...
	*/
	...
	series_base<T, K>(
		x, 
		static_cast<T>(0.5) - static_cast<T>(std::numbers::pi) * sin(x) * static_cast<T>(0.25)
	)
	{
		...
		if (x < static_cast<T>(0) || x > static_cast<T>(0.5 * std::numbers::pi) || !isfinite(x)) {
	...
	{
		const K a = static_cast<K>(fma(2, n, 1));
		return cos(static_cast<T>(2) * (this->x) * static_cast<T>(n + 1)) / static_cast<T>(a * (a + 2)); // (48.2) [Rows.pdf]
	}
	...
	*/
	...
	series_base<T, K>(
		x, 
		log(static_cast<T>(0.5) * ( static_cast<T>(1) + [&]{
			if constexpr(std::is_same<T, float_precision>::value){
				return sqrt(static_cast<T>(1) + x * x);
			} else {
				return std::hypot(static_cast<T>(1),x);
			}
		}()))
	)
	{
	...
	constexpr T ln_1plussqrt1plusxsquare_minus_ln_2_series<T, K>::operator()(K n) const
	{
		...
		return (this->minus_one_raised_to_power_n(n) * static_cast<T>(this->fact(b - 1)) * pow(this->x, static_cast<T>(b))) /
		static_cast<T>(pow(static_cast<K>(2), b) * this->fact(a) * this->fact(a)); // (49.3) [Rows.pdf]
	}
	...
	*/
	...
	ln_cosx_series<T, K>::ln_cosx_series(T x) : series_base<T, K>(x, log(cos(x)))
	{
		...
		if (abs(x) >= static_cast<T>(0.5 * std::numbers::pi) || !isfinite(x)) {
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	ln_sinx_minus_ln_x_series<T, K>::ln_sinx_minus_ln_x_series(T x) : 
	series_base<T, K>(
		x, 
		log(sin(x)) / x
	)
	...
	*/
	...
	series_base<T, K>(
		x, 
		static_cast<T>(0.125 * std::numbers::pi) * cos(x) * cos(x) - cos(x) / static_cast<T>(3)
	)
	{
	...
	{
		const K temp = static_cast<K>(fma(2, n, 1));
		return this->minus_one_raised_to_power_n(n) * cos(temp + static_cast<T>(2)) * this->x / 
	...
	*/
	...
	constexpr T sqrt_oneminussqrtoneminusx_div_x_series<T, K>::operator()(K n) const
	{
		const K a = 2 * n;
		const K b = 2 * a;
		return static_cast<T>(this->fact(b)) * pow(this->x, static_cast<T>(n)) / sqrt(static_cast<T>(2)) / static_cast<T>(pow(2, b) * this->fact(a) * this->fact(a + 1)); // (53.1) [Rows.pdf]
	}
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	arcsin_x_minus_x_series<T, K>::arcsin_x_minus_x_series(T x) : series_base<T, K>(x, static_cast<T>(asin(x)) - x)
	{
	...
	{
		const K a = static_cast<K>(fma(2, n, 1));
		return static_cast<T>(this->double_fact(a)) * pow(this->x, static_cast<T>(a+2)) / (static_cast<T>(this->double_fact(a+1)) * static_cast<T>(a+2)); // (55.3) [Rows.pdf]
	}
	...
	*/
	...
	{
		const T a = static_cast<T>(fma(2, n, 1));
		return sin(a * (this->x)) / (pow(a, static_cast<T>(3)) * static_cast<T>(0.125 * std::numbers::pi)); // (56.2) [Rows.pdf]
	}
	...
	*/
	...
	series_base<T, K>(
		x, 
		static_cast<T>(0) <= x && x <= static_cast<T>(std::numbers::pi) ? 
		sin(x) - static_cast<T>(2) / static_cast<T>(std::numbers::pi) : 
		static_cast<T>(-1)*sin(x) - static_cast<T>(2) / static_cast<T>(std::numbers::pi)
	)
	{
	...
	constexpr T abs_sin_x_minus_2_div_pi_series<T, K>::operator()(K n) const
	{
		const T a = static_cast<T>(fma(2, n, 1));
		//a_n = -4 / ( pi * (4 * n * n - 1))
		return cos(static_cast<T>(2 * n) * this->x) / static_cast<T>(-0.25 * std::numbers::pi) / static_cast<T>(fma(4,n*n,-1)); // (57.2) [Rows.pdf]
	}
	...
	*/
	...
	series_base<T, K>(
		x, 
		static_cast<T>(-std::numbers::pi) < x && x < static_cast<T>(0) ? 
		static_cast<T>(std::numbers::pi) - static_cast<T>(0.75 * std::numbers::pi): 
		static_cast<T>(std::numbers::pi) - x - static_cast<T>(0.75 * std::numbers::pi)
	)
	{
		...
		if (x <= static_cast<T>(-std::numbers::pi) || x > static_cast<T>(std::numbers::pi) || !isfinite(x)) {
	...
	*/
	...
	series_base<T, K>(
		x, 
		static_cast<T>(-3) < x && x < static_cast<T>(0) ? static_cast<T>(-0.75) : x - static_cast<T>(0.75)
	)
	{
	...
	*/
	...
	: series_base<T, K>(x, 
		x>=static_cast<T>(0) ? 
		static_cast<T>(-0.25 * std::numbers::pi) :
		static_cast<T>(-0.25 * std::numbers::pi) - x 
	)
	{
	...
	*/
	...
	: series_base<T, K>(
		x,
		abs(x) < static_cast<T>(2) ? static_cast<T>(1) / ((static_cast<T>(2) - x) * (static_cast<T>(3) + x)) : static_cast<T>(0)
	)
	{
	...
	*/
	...
	constexpr T Ci_x_series<T, K>::operator()(K n) const
	{
		...
		return this->minus_one_raised_to_power_n(n) * pow(this->x, two_n) / two_n / static_cast<T>(this->fact(2 * n)); // (65.1) [Rows.pdf], (1 - 2 * (n & 1)) = (-1)^{n}
	}
	...
	*/
	...
	Riemann_zeta_func_series<T, K>::Riemann_zeta_func_series(T x) : series_base<T, K>(x)
	{
		...
		if (x <= static_cast<T>(1) || !isfinite(x)) {
			this->throw_domain_error("x must be > 1");
		}
	}
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	Riemann_zeta_func_xmin1_div_Riemann_zeta_func_x_series<T, K>::Riemann_zeta_func_xmin1_div_Riemann_zeta_func_x_series(T x) : 
	series_base<T, K>(x)
	{
	...
	constexpr T Riemann_zeta_func_xmin1_div_Riemann_zeta_func_x_series<T, K>::operator()(K n) const
	{
		...
		const K a = n;
		return this->phi(a) / pow(static_cast<T>(a), this->x); // (67.3) [Rows.pdf]
	}
	...
	*/
	...
	constexpr T K_x_series<T, K>::operator()(K n) const
	{
		...
		T coeff = static_cast<T>(this->double_fact(2*n-1) / this->double_fact(2*n));
		coeff*=coeff;
		coeff*=pow(this->x, static_cast<T>(2*n)); //pow умер

		return static_cast<T>(std::numbers::pi * 0.5) * coeff; // (71.2) [Rows.pdf]
	}
	...
	*/
	...
	constexpr T E_x_series<T, K>::operator()(K n) const
	{
		...
		return static_cast<T>(-std::numbers::pi * 0.5) * coeff * coeff * pow(this->x, static_cast<T>(2*n)) / static_cast<T>(2*n-1); // (72.1) [Rows.pdf]
	}
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	sqrt_1plusx_series<T, K>::sqrt_1plusx_series(T x) : series_base<T, K>(x, sqrt(static_cast<T>(1) + x))
	{
	...
	constexpr T sqrt_1plusx_series<T, K>::operator()(K n) const
	{
		...
		return this->minus_one_raised_to_power_n(n+1) * coeff * pow(this->x, static_cast<T>(n)) / static_cast<T>(fma(2,n,1)) / static_cast<T>(2*n-1); // (73.1) [Rows.pdf], (1 - 2 * (n & 1)) = (-1)^{n}
	}
	...
	*/
	...
	Lambert_W_func_series<T, K>::Lambert_W_func_series(T x) : series_base<T, K>(x)
	{
		...
		if (abs(x) >= static_cast<T>(1.0 / std::numbers::e) || !isfinite(x)) {
	...
	*/
	...
	constexpr T Incomplete_Gamma_func_series<T, K>::operator()(K n) const
	{
		...
		return pow(this->x, static_cast<T>(n) + s) * exp(-this->x) / coeff; // (75.1) [Rows.pdf]
	}
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	pi_series<T, K>::pi_series(T x) : series_base<T, K>(x, x * static_cast<T>(std::numbers::pi))
	{
		this->series_name = "π";
	...
	{
		return this->minus_one_raised_to_power_n(n) * this->x * sqrt(static_cast<T>(12)) / static_cast<T>(fma(2,n,1)) / static_cast<T>(pow(3,n)); // (78.1) [Rows.pdf], (n + 1) % 2 is the same (n + 1) & 1
	}
	...
	*/
	...
	public:
		...
		*/
		...
		/**	return this->x * static_cast<T>(std::sqrt(12) * (1 - 2 * ((n + 1) % 2)) / (pow(3, n) * (2 * n + 1))); // (78.1) [Rows.pdf]
	...
	x_min_sqrt_x_series<T, K>::x_min_sqrt_x_series(T x) : series_base<T, K>(x, x - sqrt(x))
	{
		...
		if (x <= static_cast <T>(0) || x >= static_cast <T>(1) || !isfinite(x)) {
			this->throw_domain_error("x must be in (0, 1)");
		}
	}
	...
	{
		if (n == 0 || n == 1)
			return (static_cast<T>(1) - this->binomial_coefficient(static_cast<T>(0.5), n)) * pow(static_cast<T>(-1) + this->x, static_cast<T>(n)); // (79.1) [Rows.pdf]
		return static_cast<T>(-1) * this->binomial_coefficient(static_cast<T>(0.5), n) * pow(static_cast<T>(-1) + this->x, static_cast<T>(n)); // (79.1) [Rows.pdf]
	}
	...
	*/
	...
	{
		const T a = static_cast<T>(2 * n + 1);
		return series_base<T, K>::minus_one_raised_to_power_n(n) * static_cast<T>(pow(this->x, static_cast<T>(2) * a)) / a; // (80.2) [Rows.pdf]
	}
	...
	*/
	...
	{
		const K a = n + 1;
		return series_base<T, K>::minus_one_raised_to_power_n(a + 1) * pow(this->x, static_cast<T>(static_cast <K>(4) * a)) / static_cast<T>(a); // (81.3) [Rows.pdf]
	}
	...
	*/
	...
	constexpr T arctan_x3_series<T, K>::operator()(K n) const
	{
		...
		return series_base<T, K>::minus_one_raised_to_power_n(n) * (pow(this->x, static_cast<T>(3 * a))) / static_cast<T>(a); // (83.2) [Rows.pdf],
	}
	...
	*/
	...
	{
		const K a = static_cast<K>(fma(static_cast <K>(2), n, static_cast <K>(1)));
		return static_cast<T>(this->fact(a - static_cast <K>(1))) * pow(this->x, static_cast<T>(2 * a))/ static_cast<T>(pow(static_cast <K>(4), n) * pow(this->fact(n), 2 * a)); // (84.1) [Rows.pdf]
	}
	...
	*/
	...
	{
		const K a = static_cast<K>(fma(static_cast <K>(2), n, static_cast <K>(1)));
		return series_base<T, K>::minus_one_raised_to_power_n(n) * static_cast<T>(this->double_fact(a)) * pow(this->x, static_cast<T>(a + static_cast <K>(2))) / static_cast<T>((this->double_fact(a + static_cast <K>(1)) * (a + static_cast <K>(2))));  // (87.2) [Rows.pdf]
	}
	...
	*/
	...
	{
		const K a = 2 * n;
		return series_base<T, K>::minus_one_raised_to_power_n(n) * pow(this->x, static_cast<T>(2 * a)) / static_cast<T>(this->fact(a)); // (88.2) [Rows.pdf]
	}
	...
	*/
	...
	{
		const K a = static_cast<K>(fma(2, n, 1));
		return pow(this->x, static_cast<T>(2 * a)) / static_cast<T>(this->fact(a)); // (89.2) [Rows.pdf]
	}
	...
	*/
	...
	{
		const K a = 2 * n + 2;
		return series_base<T, K>::minus_one_raised_to_power_n(n + 1) * static_cast<T>(pow(3, a)) * pow(this->x, static_cast<T>(a - 2)) / static_cast<T>(this->fact(a));
	}
	...
	*/
	...
	{
		return this->minus_one_raised_to_power_n(n) * pow(this->x / static_cast <T>(2), 2 * n + this->a) / static_cast<T>(this->fact(n) * tgamma(this->a + static_cast<T>(n + 1))); // (95.1) [Rows.pdf]
	}
	...
	*/
	...
	template <FloatLike T, std::unsigned_integral K>
	one_div_sqrt2_sin_xdivsqrt2_series<T, K>::one_div_sqrt2_sin_xdivsqrt2_series(T x) : series_base<T, K>(x, static_cast<T>((1 / std::sqrt(2))* sin(x / std::sqrt(2))))
	{
	...
	*/
	...
	gamma_series<T, K>::gamma_series(T t, T x) : series_base<T, K>(x), t(t), x(x)
	{
		...
		if (t <= static_cast <T>(0) || !isfinite(x) || !isfinite(t)) {
	...
	{
		T a_k_n = a_k(n);
		return static_cast<T>(a_k_n * pow(this->t, static_cast<T>(n)) * tgamma(a_k_n)); // (102.1) [Rows.pdf]
	}
	...
	*/
	...
	T reccurent_testing_series<T, K>::access_row(K n)
	{
		...
		for (K i = old_size; i <= n; ++i)
			series_vec.push_back(series_vec[i - 1] * (this->x * this->x) / static_cast<T>(i * fma(4, i, 2)));
