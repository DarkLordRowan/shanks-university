...
3.2)Если выбрана трансформация 10, то надо выбрать параметр Beta.
3.3)Если выбрана трансформация 13, то надо выбрать какую использовать 0 - classic(не требует доп параметров), 1 - gamma(требует доп параметр), 2 - gamma-rho(требует 2 доп параметра)
4)Выбираем функцию
5)Вводим n
6)Вводим order
...
def main():
    exec_file = ""
    if (os.name == "nt"):
        exec_file = r"C:\Users\DLR_ACER\HW_Mirea\shanks-university\x64\Debug\shanks_transformation.exe" # Путь к exe файлу
    else:
