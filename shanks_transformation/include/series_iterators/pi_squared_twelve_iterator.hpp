...
public:
	...
	T sum() const override{
		return this->x * static_cast<T>(std::numbers::pi) * static_cast<T>(std::numbers::pi)/static_cast<T>(12);
	}
	...
	T next() override {
		...
		this->current_state = this->x * utils::minus_one_raised_to_power_n<T, K>(this->n) / static_cast<T>((this->n+1) * (this->n+1));
		this->n += 1;
