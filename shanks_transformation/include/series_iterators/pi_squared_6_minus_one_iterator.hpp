...
/**
* @brief Maclaurin series of x * (pi^2/6-1) function
...
public:
	...
	T sum() const override{
		return this->x * (static_cast<T>(std::numbers::pi) * static_cast<T>(std::numbers::pi)/static_cast<T>(6) - static_cast<T>(1));
	}
	...
	T next() override {
		...
		this->current_state = this->x / static_cast<T>((this->n+1) * (this->n+1) * (this->n+2));
		this->n += 1;
