...
public:
	...
	T sum() const override{
        ...
        return atan(pow(this->x, static_cast<T>(3)));
	}
	...
	T next() override {
		...
		else this->current_state *= static_cast<T>(-1) * pow(this->x, static_cast<T>(6)) * static_cast<T>(fma(2,this->n-1,1)) / static_cast<T>(fma(2,this->n,1));
