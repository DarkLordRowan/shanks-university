...
public:
	...
	T sum() const override{
		return static_cast<T>(0.599195688977) * this->x;
	}
	...
	T next() override {
		...
		this->current_state = log(static_cast<T>(1) + 
		pow(n1, n1_2 + n1 * static_cast<T>(0.5)) / (pow(static_cast<T>(utils::fact<K>(this->n+1)), n1) * exp(n1_2))) * this->x;
		this->n += 1;
