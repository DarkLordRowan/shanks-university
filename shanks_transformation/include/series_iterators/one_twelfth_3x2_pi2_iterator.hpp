...
public:
	...
	T sum() const override{
		return static_cast<T>(1) / static_cast<T>(12) * (static_cast<T>(3) * this->x * this->x - static_cast<T>(std::numbers::pi) * static_cast<T>(std::numbers::pi));
	}
	...
	T next() override {
		...
		cos(static_cast<T>(this->n+1) * this->x) / static_cast<T>((this->n + 1) * (this->n + 1));
		this->n += 1;
