...
public:
	...
	*/
	explicit pi_six_min_half_series() : series_base<T, K>("pi_six_min_half_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x * (static_cast<T>(std::numbers::pi)/static_cast<T>(6) - static_cast<T>(0.5));
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> pi_six_min_half_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(0); j < vecSize; ++j){
		vecAn[j] += utils::minus_one_raised_to_power_n<T, K>(j) * x / static_cast<T>(fma(6,j,5) * fma(6,j,7));
