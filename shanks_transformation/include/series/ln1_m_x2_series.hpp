...
public:
	...
	*/
	explicit ln1_m_x2_series() : series_base<T, K>("ln1_m_x2_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return log(static_cast<T>(1) - x * x);
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> ln1_m_x2_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(1); j < vecSize; ++j){
		vecAn[j] += vecAn[j-static_cast<K>(1)] * x * x * static_cast<T>(j) / static_cast<T>(j+1);
