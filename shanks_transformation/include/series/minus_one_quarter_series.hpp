...
public:
	...
	*/
	explicit minus_one_quarter_series() : series_base<T, K>("minus_one_quarter_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x * static_cast<T>(-0.25);
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> minus_one_quarter_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
