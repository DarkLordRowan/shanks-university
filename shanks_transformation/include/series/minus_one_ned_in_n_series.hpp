...
public:
	...
	*/
	explicit minus_one_ned_in_n_series() : series_base<T, K>("minus_one_ned_in_n_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x * static_cast<T>(-0.78343051);
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> minus_one_ned_in_n_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
