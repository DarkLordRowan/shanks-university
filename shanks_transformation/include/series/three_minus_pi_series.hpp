...
public:
	...
	*/
	explicit three_minus_pi_series() : series_base<T, K>("three_minus_pi_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x * (static_cast<T>(3) - static_cast<T>(std::numbers::pi));
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> three_minus_pi_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(0); j < vecSize; ++j){
		vecAn[j] += utils::minus_one_raised_to_power_n<T, K>(j+1) * x / static_cast<T>((j+1)*(j+2)*fma(2,j,3));
