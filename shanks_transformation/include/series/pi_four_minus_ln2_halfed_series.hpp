...
public:
	...
	*/
	explicit pi_four_minus_ln2_halfed_series() : series_base<T, K>("pi_four_minus_ln2_halfed_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x * (static_cast<T>(std::numbers::pi * 0.25) - log(static_cast<T>(2)) * static_cast<T>(0.5));
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> pi_four_minus_ln2_halfed_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(0); j < vecSize; ++j){
		vecAn[j] += utils::minus_one_raised_to_power_n<T, K>(j) * x * (static_cast<T>(1) / static_cast<T>(fma(2,j,1)) - static_cast<T>(1) / static_cast<T>(fma(2,j,2)));
