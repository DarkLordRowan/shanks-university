...
public:
	...
	*/
	explicit arctanh_x2_series() : series_base<T, K>("arctanh_x2_series") {};
...
template<AcceptedLike T, UnsignedIntLike K>
series_result<T> arctanh_x2_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(1); j < vecSize; ++j){
		vecAn[j] += vecAn[j-1] * x_4 * static_cast<T>(fma(2,j-1,1)) / static_cast<T>(fma(2,j,1));
