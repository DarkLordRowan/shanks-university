...
public:
	...
	*/
	explicit pi_series() : series_base<T, K>("pi_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x * static_cast<T>(std::numbers::pi);
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> pi_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
    ...
    vecAn[0] = x * sqrt(static_cast<T>(12));
    vecSn[0] = x * sqrt(static_cast<T>(12));


	for(K j = static_cast<K>(1); j < vecSize; ++j){
		vecAn[j] += static_cast<T>(-1) * vecAn[j-static_cast<K>(1)] * static_cast<T>(fma(2,j-1,1)) / static_cast<T>(3 * fma(2,j,1));
