...
public:
	...
	*/
	explicit half_asin_two_x_series() : series_base<T, K>("half_asin_two_x_series") {};
	...
	inline constexpr bool check_domain(const T& x){
        ...
        if constexpr (isComplexLike<T>::value){
    		return !isfinite(x.real()) || !isfinite(x.imag()) || abs(x) > abs(static_cast<T>(0.5));
        } else {
...
template<AcceptedLike T, UnsignedIntLike K>
series_result<T> half_asin_two_x_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
