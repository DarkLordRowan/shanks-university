...
public:
	...
	*/
	explicit pi_cubed_32_series() : series_base<T, K>("pi_cubed_32_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x * pow(static_cast<T>(std::numbers::pi), static_cast<T>(3)) / static_cast<T>(32);
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> pi_cubed_32_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(0); j < vecSize; ++j){
		vecAn[j] += utils::minus_one_raised_to_power_n<T, K>(j) * x / static_cast<T>(fma(2,j,1) * fma(2,j,1) * fma(2,j,1));
