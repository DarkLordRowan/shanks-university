...
public:
	...
	*/
	explicit one_twelfth_x2_pi2_series() : series_base<T, K>("one_twelfth_x2_pi2_series") {};
	...
	inline constexpr T calculate_sum(const T& x){
		...
		return x / static_cast<T>(12) * (x*x - static_cast<T>(std::numbers::pi) * static_cast<T>(std::numbers::pi));
	}

};

template<AcceptedLike T, UnsignedIntLike K>
series_result<T> one_twelfth_x2_pi2_series<T, K>::generate_series(
    const T& x , 
	const K vecSize, 
	...
	for(K j = static_cast<K>(0); j < vecSize; ++j){
		vecAn[j] += utils::minus_one_raised_to_power_n<T, K>(j+1) * sin(static_cast<T>(j+1) * x) / static_cast<T>((j+1)*(j+1)*(j+1));
