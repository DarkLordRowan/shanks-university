...
protected:
	...
	T gamma_in_use = static_cast<T>(DEFAULT_GAMMA);											///< Positive real parameter such that gamma >= order - 1
...
public:
	...
	explicit levin_sidi_m_algorithm(
		...
		const T& gamma_to_use = static_cast<T>(DEFAULT_GAMMA)
...
complex_precision<T> levin_sidi_m_algorithm<complex_precision<T>, K>::operator()(
	...
	for (K j = static_cast<K>(0); j <= n; ++j) {
		...
		rest *= Complex(binomial_coef) * Complex(n - j);
		rest *= Complex(up);										// Multiply by Pochhammer ratio term
		...
		rest *= remainder->operator()(
			...
			order + j,
			data.an,
			Complex(-gamma_in_use-static_cast<T>(n))
		);
