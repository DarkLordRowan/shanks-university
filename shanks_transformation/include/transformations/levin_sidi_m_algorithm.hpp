...
protected:
	...
	param_type gamma_in_use = static_cast<param_type>(DEFAULT_GAMMA);   ///< Positive real parameter such that gamma >= order - 1
...
public:
	...
	explicit levin_sidi_m_algorithm(
		...
		const param_type& gamma_to_use = static_cast<param_type>(DEFAULT_GAMMA)
...
T levin_sidi_m_algorithm<T, K>::operator()(
	...
	if(gamma_in_use - static_cast<param_type>(n) - static_cast<param_type>(1) < static_cast<param_type>(0)){
		throw std::domain_error("gamma cannot be lesser than n - 1");
	}

	T numerator, denominator, rest;
	rest = numerator = denominator = static_cast<T>(0);
	param_type up, down, down_coef, up_coef;
	up = down = static_cast<param_type>(1);
	down_coef = up_coef = static_cast<param_type>(0);
	...
	down_coef += gamma_in_use + static_cast<param_type>(order + static_cast<K>(2));
	up_coef   += down_coef - static_cast<param_type>(n);

	// Compute (γ+k+2)_{n-1} = ∏_{m=0}^{n-2} (γ+k+2+m)
	// Compute (γ+k+1)_{n} = ∏_{m=0}^{n-1} (γ+k+1+m)
	for (K m = static_cast<K>(0); m + static_cast<K>(1) < n; ++m) {
		up   *= (up_coef   + static_cast<param_type>(m));
		down *= (down_coef + static_cast<param_type>(m));
	}
	up /= down;

	// Update coefficients for the inner product terms
	down_coef = gamma_in_use + static_cast<param_type>(order + static_cast<K>(1));
	up_coef   = down_coef - static_cast<param_type>(n + static_cast<K>(1));
	// Main summation loop
	...
	for (K j = static_cast<K>(0); j <= n; ++j) {
		...
		rest *= static_cast<T>(utils::binomial_coefficient<K>(n, j));
		rest *= static_cast<T>(up);										// Multiply by Pochhammer ratio term
		rest /= static_cast<T>(j + static_cast<K>(1));  // Multiply by 1/(j+1) factor

		up /= (  up_coef + static_cast<param_type>(j));			// Update Pochhammer ratio for next iteration
		up *= (down_coef + static_cast<param_type>(j));			// (γ+k+1-j)_{j}/(γ+k+2-n)_{j} → (γ+k+1-j)_{j+1}/(γ+k+2-n)_{j+1}
		...
		rest *= remainder->operator()(
			...
			order + j,
			data.an,
			static_cast<T>(-gamma_in_use-static_cast<param_type>(n))
		);
