...
protected:
	...
	param_type beta_in_use = static_cast<param_type>(1);			///< Parameter for u-variant transformation (β > 0). Default value is 1.0.
...
public:
	...
	void update_beta(const param_type& new_beta){
		beta_in_use = (new_beta > static_cast<param_type>(0) ? new_beta : static_cast<param_type>(1));
	}
...
inline T levin_algorithm<T, K>::calc_result(
	...
	C_njk = static_cast<param_type>(0);
	...
	for (K j = static_cast<K>(0); j <= order; ++j) {
		...
		C_njk  = pow(beta_in_use + static_cast<param_type>(n + j     + static_cast<K>(1)), static_cast<param_type>(order - static_cast<K>(1)));
		C_njk /= pow(beta_in_use + static_cast<param_type>(n + order + static_cast<K>(1)), static_cast<param_type>(order - static_cast<K>(1)));
		...
		rest*= remainder->operator()(
            ...
            n + j,
            data.an,
            static_cast<T>((remainder_type_in_use == remainder_type::u_type ? beta_in_use : static_cast<param_type>(1)))
        );
...
inline T levin_algorithm<T, K>::calc_result_rec(
	...
	param_type scale = static_cast<param_type>(0);
	...
	for (K i = static_cast<K>(1); i <= order; ++i)
		for (K j = static_cast<K>(0); j <= order - i; ++j) {
			...
			scale = (beta_in_use + static_cast<param_type>(n + j));
			scale*= pow(static_cast<param_type>(1) - static_cast<param_type>(1) / (beta_in_use + static_cast<param_type>(n + j + i + 1)), static_cast<param_type>(i));
			scale/= (beta_in_use + static_cast<param_type>(n + j + i));

			Denom[j] = fma(-static_cast<T>(scale),Denom[j],Denom[j+static_cast<K>(1)]);
              Num[j] = fma(-static_cast<T>(scale),  Num[j],  Num[j+static_cast<K>(1)]);
		}
