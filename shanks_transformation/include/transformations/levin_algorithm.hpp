...
inline T levin_algorithm<T, K>::calc_result(
	...
	T numerator, denominator, C_njk, rest;
	numerator = denominator = C_njk = rest = static_cast<T>(0);
...
inline complex_precision<T> levin_algorithm<complex_precision<T>, K>::calc_result(
	...
	for (K j = static_cast<K>(0); j <= order; ++j) {
		...
		// Compute (n+j+1)^{k-1}/(n+k+1)^{k-1}
		C_njk  = pow(
			beta_in_use + static_cast<T>(n + j     + static_cast<K>(1)), 
		...
		rest*= remainder->operator()(
            ...
            n + j,
            data.an,
            (remainder_type_in_use == remainder_type::u_type ? beta_in_use : static_cast<Complex>(1))
        );

		rest *= static_cast<Complex>(C_njk);
...
inline complex_precision<T> levin_algorithm<complex_precision<T>, K>::calc_result_rec(
	...
	if constexpr (std::is_floating_point<T>::value){
		...
		Denom = std::vector<Complex>( order + static_cast<K>(1), static_cast<Complex>(0));
	}
	#ifdef INC_FPRECISION
	else if constexpr (std::is_same<T, float_precision>::value){
		const size_t precision = std::max(
