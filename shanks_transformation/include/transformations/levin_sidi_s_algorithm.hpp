...
protected:
    ...
    param_type beta_in_use = static_cast<param_type>(1);                    ///< Positive real parameter (β > 0). Default value is 1.0.
...
public:
    ...
    explicit levin_sidi_s_algorithm(
        ...
        const param_type& beta_to_use = static_cast<param_type>(1)
    ...
    void update_beta(const param_type& new_beta){ beta_in_use = (new_beta > static_cast<param_type>(0) ?  new_beta : static_cast<param_type>(1)); }
...
inline T levin_sidi_s_algorithm<T, K>::calc_result(
    ...
    param_type up_pochamer, down_pochamer;
    numerator = denominator = rest = static_cast<T>(0);
    up_pochamer = down_pochamer = static_cast<param_type>(0);
    ...
    for (K j = static_cast<K>(0); j <= order; ++j){
        ...
        up_pochamer = down_pochamer = static_cast<param_type>(1);
        ...
        for (K i = static_cast<K>(0); i < order - static_cast<K>(1); ++i){
            up_pochamer   *= (beta_in_use + static_cast<param_type>(n + j     + i));
            down_pochamer *= (beta_in_use + static_cast<param_type>(n + order + i));
        }
        ...
        rest *= remainder->operator()(
            ...
            n + j,
            data.an,
            (remainder_type_in_use == remainder_type::u_type ? static_cast<T>(beta_in_use) : static_cast<T>(1))
        );
...
inline T levin_sidi_s_algorithm<T, K>::calc_result_rec(
    ...
    std::vector<T>   Num(order + static_cast<K>(1),  static_cast<T>(0.0));
    std::vector<T> Denom(order + static_cast<K>(1),  static_cast<T>(0.0));

    param_type scale1, scale2;
    scale1 = scale2 = static_cast<param_type>(0);
    ...
    for (K i = static_cast<K>(1); i <= order; ++i)
        for(K j = static_cast<K>(0); j <= order - i; ++j){
            ...
            scale1 = beta_in_use + static_cast<param_type>(n + i + j);
            scale1*= (scale1 + static_cast<param_type>(1));

            scale2 = scale1 + static_cast<param_type>(n + i);
            scale2*= (scale2 + static_cast<param_type>(1));

            // Apply recurrence: E_k^{(n)} = E_{k-1}^{(n)} - g_{k-1,k}^{(n)} * ΔE_{k-1}^{(n)} / Δg_{k-1,k}^{(n)}
            Denom[j] = fma(static_cast<T>(-scale1),Denom[j]/static_cast<T>(scale2),Denom[j+static_cast<K>(1)]);
              Num[j] = fma(static_cast<T>(-scale1),  Num[j]/static_cast<T>(scale2),  Num[j+static_cast<K>(1)]);
        }
