...
protected:
    ...
    T beta_in_use = static_cast<T>(1);                                                 ///< Positive real parameter (β > 0). Default value is 1.0.
...
inline T levin_sidi_s_algorithm<T, K>::calc_result(
    ...
    T numerator, denominator, rest, up_pochamer, down_pochamer;
    numerator = denominator = rest = up_pochamer = down_pochamer = static_cast<T>(0);
    ...
    if constexpr (std::is_same<T, float_precision>::value){
        ...
        up_pochamer.precision(precision); down_pochamer.precision(precision);
    }
    #endif
...
inline complex_precision<T> levin_sidi_s_algorithm<complex_precision<T>, K>::calc_result(
    ...
    numerator = denominator = rest = static_cast<Complex>(0);
    up_pochamer = down_pochamer = static_cast<T>(0);
    ...
    for (K j = static_cast<K>(0); j <= order; ++j){
        ...
        rest *= Complex(utils::binomial_coefficient<K>(order, j));
        ...
        rest *= Complex(up_pochamer / down_pochamer);  // Multiply by Pochhammer ratio
        rest *= remainder->operator()(
            n + j,        // Multiply by remainder term 1/R_{n+j}
            n + j,
            data.an,
            Complex(remainder_type_in_use == remainder_type::u_type ? beta_in_use : static_cast<T>(1))
        );
