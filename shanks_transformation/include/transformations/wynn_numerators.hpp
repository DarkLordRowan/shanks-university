...
public:
	...
	virtual T operator()(K n, K order, const std::vector<T>& an, T gamma = static_cast<T>(-1), T rho = static_cast<T>(1)) const = 0;
...
public:
	...
	T operator()(const K n, const K order, const std::vector<T>& an, const T gamma = static_cast<T>(-1), const T rho = static_cast<T>(1)) const {
