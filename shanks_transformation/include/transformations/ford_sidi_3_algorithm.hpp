...
class ford_sidi_3_algorithm<complex_precision<T>, K> final : public series_acceleration<complex_precision<T>, K>{
public:
    ...
    *        The series object should provide access to terms and partial sums.
    */
    explicit ford_sidi_3_algorithm() : series_acceleration<complex_precision<T>, K>("ford sidi 3") {};
...
template <FloatLike T, UnsignedIntLike K>
complex_precision<T> ford_sidi_3_algorithm<complex_precision<T>, K>::operator()(
    const K n, 
    const K order, 
    const series_result<complex_precision<T>>& data
) const {
    ...
    // FSA sequence: Stores accelerated partial sums
    std::vector<Complex> FSA;
    // FSI sequence: Stores normalization factors
    std::vector<Complex> FSI;
    // FSG matrix: Stores intermediate transformation values
    std::vector<std::vector<Complex>> FSG;
    // For theory, see: Ford & Sidi (1987), Eq. (2.3) - Recursive coefficient scaling
    // Te = 1/n used for recursive computation of G sequence
    Complex D;
    T Te;
    D = static_cast<Complex>(0); Te = static_cast<T>(1);
    if constexpr (std::is_floating_point<T>::value){
        G = std::vector<Complex>(m + static_cast<K>(1), static_cast<Complex>(0));
        FSA = std::vector<Complex>(m + static_cast<K>(1), static_cast<Complex>(0));
        FSI = std::vector<Complex>(m + static_cast<K>(1), static_cast<Complex>(0));
        FSG = std::vector<std::vector<Complex>>(m + static_cast<K>(2), std::vector<Complex>(m+static_cast<K>(1), static_cast<Complex>(0)));
    }
    #ifdef INC_FPRECISION
    else if constexpr (std::is_same<T, float_precision>::value){
        const size_t precision = std::max(
        ...
        Te.precision(precision); D.ref_real()->precision(precision); D.ref_imag()->precision(precision);
        G = std::vector<Complex>(m + static_cast<K>(1), D);
        FSA = std::vector<Complex>(m + static_cast<K>(1), D);
        FSI = std::vector<Complex>(m + static_cast<K>(1), D);
        FSG = std::vector<std::vector<Complex>>(m + static_cast<K>(2), std::vector<Complex>(m+static_cast<K>(1), D));
    }
    #endif
    Te /= static_cast<T>(n);
    ...
    // G[k] = (1/n) * G[k-1] for k = 1, 2, ..., m
    for (K k = static_cast<K>(1); k <= m; ++k)
        G[k] = Complex(Te) * G[k - static_cast<K>(1)];
