...
complex_precision<T> wynn_epsilon_3_algorithm<complex_precision<T>, K>::operator()(
    ...
    T abs_error    = static_cast<T>(0.0);    ///< Absolute error estimate for current data.
    Complex resla  = static_cast<Complex>(0.0);        ///< Previous result for error comparison.
    K newelm, num, NUM, K1, ib, ie, in; // Loop indices and counters.
    Complex RES    = static_cast<Complex>(0.0);
    Complex E0, E1, E2, E3;
    E0 = E1 = E2 = E3 = static_cast<Complex>(0);
    Complex DELTA1, DELTA2, DELTA3;
    DELTA1 = DELTA2 = DELTA3 = static_cast<Complex>(0);
    T ERROR, ERR1, ERR2, ERR3;
    ERROR = ERR1 = ERR2 = ERR3 = static_cast<T>(0);
    T TOL1, TOL2, TOL3;
    TOL1 = TOL2 = TOL3 = static_cast<T>(0);
    Complex SS = static_cast<Complex>(0.0);

    // For theory, see: Wynn (1956), Section 3: Algorithm and lozenge diagram.
    // The epsilon table e[0..N+2] stores intermediate values εₛ⁽ⁿ⁾.
    // For theory, see: Wynn (1956), Section 3: Algorithm and lozenge diagram.
    // The epsilon table e[0..N+2] stores intermediate values εₛ⁽ⁿ⁾.
    std::vector<Complex> e; //First N eliments of epsilon table + 2 elements for math

    if constexpr (is_standart_types<T>::value){
        e = std::vector<Complex>(
            N + static_cast<K>(3),
            static_cast<T>(0.0)  
        );
    }
