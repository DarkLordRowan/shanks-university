...
class brezinski_theta_algorithm<complex_precision<T>,K> final : public series_acceleration<complex_precision<T>, K>
{
public:

    explicit brezinski_theta_algorithm() : series_acceleration<complex_precision<T>, K>("brezinski theta algorithm") {}
...
template <FloatLike T, UnsignedIntLike K>
complex_precision<T> brezinski_theta_algorithm<complex_precision<T>, K>::operator()(
    const K n, 
    const K order,
	const series_result<complex_precision<T>>& data
) const {
    ...
    const K base_size = static_cast<K>(3) * order / static_cast<K>(2) + static_cast<K>(1);
    std::vector<Complex> theta_odd;
    std::vector<Complex> theta_even;
    Complex delta;

    if constexpr (std::is_floating_point<T>::value){

        theta_odd = std::vector<Complex>(
            base_size,
            Complex(0)
        ); // vector for theta_(2n + 1);

        theta_even = std::vector<Complex>(
            base_size,
            Complex(0)
        ); //vector for theta_(2n), in the beginning it is theta_(-1) which is zero for all i

        delta = Complex(0); //temporary varaible
        ...
        delta = complex_precision<float_precision>(
            float_precision(0, precision),
            float_precision(0, precision)
         ); //temporary varaible
