...
#define BIND_SERIES_IMPL(snake, camel, tName, kName) \
    m.def(create_name(camel, suffix).c_str(), \
        [](K n, T x, T t, K k) { \
            using S = shanks::series::snake##_iterator<T, K>; \
            std::unique_ptr<S> it; \
            if constexpr (std::is_constructible_v<S, T, T, K>) it = std::make_unique<S>(x, t, k); \
            else if constexpr (std::is_constructible_v<S, T, T>) it = std::make_unique<S>(x, t); \
            else if constexpr (std::is_constructible_v<S, T, K>) it = std::make_unique<S>(x, k); \
            else it = std::make_unique<S>(x); \
            return std::make_pair(it->generate(n), it->get_sum()); \
        }, py::arg("n"), py::arg("x") = T(0), \
           py::arg(tName ? tName : "tParam") = T(1), \
...
#define BIND_DEFAULT(cls, name) \
    m.def(create_name(name, suffix).c_str(), \
        [](K n, K order, const series_result<T>& data) { \
            return shanks::algos::cls<T, K>()(n, order, data); \
        }, py::arg("n"), py::arg("order"), py::arg("data"))

#define BIND_REM(cls, name) \
    m.def(create_name(name, suffix).c_str(), \
        [](K n, K order, const series_result<T>& data, \
           shanks::remainders::remainder_type remainder) { \
...
#define BIND_REM_BOOL(cls, name) \
    m.def(create_name(name, suffix).c_str(), \
        [](K n, K order, const series_result<T>& data, \
           shanks::remainders::remainder_type remainder, bool useRecurrentFormula) { \
...
#define BIND_LEVIN_BASE(cls, name) \
    m.def(create_name(name, suffix).c_str(), \
        [](K n, K order, const series_result<T>& data, \
           shanks::remainders::remainder_type remainder, bool useRecurrentFormula, RealT beta) { \
            return shanks::algos::cls<T, K>(remainder, useRecurrentFormula, beta)(n, order, data); \
        }, py::arg("n"), py::arg("order"), py::arg("data"), \
           py::arg("remainder") = shanks::remainders::remainder_type::u_type, \
           py::arg("useRecurrentFormula") = false, \
...
#define BIND_RHO(cls, name) \
    m.def(create_name(name, suffix).c_str(), \
        [](K n, K order, const series_result<T>& data, \
           shanks::numerators::numerator_type numerator, RealT gamma, RealT RHO) { \
            return shanks::algos::cls<T, K>(numerator, gamma, RHO)(n, order, data); \
        }, py::arg("n"), py::arg("order"), py::arg("data"), \
