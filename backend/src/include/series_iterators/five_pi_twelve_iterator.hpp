...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	five_pi_twelve_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
     ...
     */
	T get_sum() const override{ return this->x * utils::cast<T>(std::numbers::pi * 5) / utils::cast<T>(12);}
	...
	T next(K n, T& state) const override {
		...
		if (n == 0) state = utils::cast<T>(5) * this->x / utils::cast<T>(3);
		else state = utils::cast<T>(5) / utils::cast<T>(3) *
		utils::minus_one_raised_to_power_n<T,K>(n) * this->x / utils::cast<T>(utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1)));
		return state;
	}
