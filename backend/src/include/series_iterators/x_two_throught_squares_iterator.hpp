...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	x_two_throught_squares_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
     ...
     */
	T get_sum() const override{ return this->x * utils::cast<T>(0.5);}
	...
	T next(K n, T& state) const override {
		...
		state = utils::cast<T>(2 *(n + 1)*(n + 1) - 1) * this->x / utils::cast<T>(4 * utils::pow(static_cast<size_t>(n + 1), size_t{4}) + 1);
		return state;
	}
