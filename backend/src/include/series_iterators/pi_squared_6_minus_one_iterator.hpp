...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	pi_squared_6_minus_one_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
     ...
     */
	T get_sum() const override{ return this->x * (utils::cast<T>(std::numbers::pi) * utils::cast<T>(std::numbers::pi)/utils::cast<T>(6) - utils::cast<T>(1));}
