...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	ten_minus_x_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
	...
	T next(K n, T& state) const override {
		...
		(utils::cast<T>(n+1) * utils::cast<T>(std::numbers::pi)) * utils::sin(utils::cast<T>(n+1) * utils::cast<T>(std::numbers::pi * 0.2) * this->x);
		return state;
	}
