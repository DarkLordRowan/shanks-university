...
public:
     ...
     */
	T get_sum() const override{ return utils::asinh(this->x);}
	...
	T next(K n, T& state) const override {
		...
		utils::cast<T>(utils::fma(static_cast<size_t>(2),static_cast<size_t>(n-1),static_cast<size_t>(1))) / utils::cast<T>(size_t{2} * n);
		return state;
	}
