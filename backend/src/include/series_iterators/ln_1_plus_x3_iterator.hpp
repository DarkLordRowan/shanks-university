...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	ln_1_plus_x3_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
	...
	T next(K n, T& state) const override {
		...
		if (n == 0) state = utils::pow(this->x, utils::cast<T>(3.0));
		else state *= utils::cast<T>(-1.0) * utils::pow(this->x, utils::cast<T>(3.0)) *
