...
public:
     ...
     */
	T get_sum() const override{ return this->x * (utils::cast<T>(4) * utils::log(utils::cast<T>(2)) - utils::cast<T>(3));}
	...
	T next(K n, T& state) const override {
		...
		state = utils::minus_one_raised_to_power_n<T,K>(n+1) * this->x / utils::cast<T>((n+1)*(n+1)*(n+2)*(n+2));
		return state;
	}
