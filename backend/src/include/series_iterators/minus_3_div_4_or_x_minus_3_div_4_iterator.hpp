...
public:
	...
	T next(K n, T& state) const override {
		...
		state = utils::cast<T>(-2) /
		(piDiv3 * piDiv3 * utils::cast<T>(3 * utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1)) * utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1)))) *
		utils::cos(shifted_x) + utils::minus_one_raised_to_power_n<T, K>(n) / (piDiv3 * utils::cast<T>(n+1)) * utils::sin(shifted_x);
		return state;
	}
