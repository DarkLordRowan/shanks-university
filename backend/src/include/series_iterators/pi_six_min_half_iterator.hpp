...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	pi_six_min_half_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
     ...
     */
	T get_sum() const override{ return this->x * (utils::cast<T>(std::numbers::pi)/utils::cast<T>(6) - utils::cast<T>(0.5));}
	...
	T next(K n, T& state) const override {
		...
		utils::cast<T>(utils::fma(static_cast<size_t>(6),static_cast<size_t>(n),static_cast<size_t>(5)) * utils::fma(static_cast<size_t>(6),static_cast<size_t>(n),static_cast<size_t>(7)));
		return state;
	}
