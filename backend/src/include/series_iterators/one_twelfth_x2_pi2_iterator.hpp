...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	one_twelfth_x2_pi2_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
     ...
     */
	T get_sum() const override{ return this->x / utils::cast<T>(12) * (this->x * this->x - utils::cast<T>(std::numbers::pi) * utils::cast<T>(std::numbers::pi));}
	...
	T next(K n, T& state) const override {
		...
		utils::sin(utils::cast<T>(n+1) * this->x) / utils::cast<T>(utils::pow(static_cast<size_t>(n+1), static_cast<size_t>(3)));
		return state;
	}
