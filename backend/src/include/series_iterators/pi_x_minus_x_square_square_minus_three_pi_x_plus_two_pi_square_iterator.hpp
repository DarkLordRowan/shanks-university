...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	pi_x_minus_x_square_square_minus_three_pi_x_plus_two_pi_square_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
	...
	T next(K n, T& state) const override {
		...
		state = utils::cast<T>(8) * utils::sin(utils::cast<T>(utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1))) * this->x) /
		(utils::cast<T>(std::numbers::pi) * utils::cast<T>(utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1)) *
		utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1))*utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1))));
		return state;
	}
