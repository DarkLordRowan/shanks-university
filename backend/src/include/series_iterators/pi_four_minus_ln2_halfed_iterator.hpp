...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	pi_four_minus_ln2_halfed_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
     ...
     */
	T get_sum() const override{ return this->x * (utils::cast<T>(std::numbers::pi * 0.25) - utils::log(utils::cast<T>(2)) * utils::cast<T>(0.5)); }
	...
	T next(K n, T& state) const override {
		...
		(utils::cast<T>(1) / utils::cast<T>(utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1))) - utils::cast<T>(1) /
