...
public:
     ...
     */
	T get_sum() const override{ return this->x * utils::log(utils::cast<T>(13)/utils::cast<T>(7)) / utils::cast<T>(7);}
	...
	T next(K n, T& state) const override {
		...
		if (n == 0) state = utils::cast<T>(6) * this->x / utils::cast<T>(49);
		else state *= utils::cast<T>(-1) * utils::cast<T>(6 * n) / utils::cast<T>(utils::fma(static_cast<size_t>(7),static_cast<size_t>(n),static_cast<size_t>(7)));
		return state;
	}
