...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	one_third_pi_squared_m_nine_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
     ...
     */
	T get_sum() const override{ return this->x * (utils::cast<T>(std::numbers::pi) * utils::cast<T>(std::numbers::pi) - utils::cast<T>(9)) / utils::cast<T>(3);}
