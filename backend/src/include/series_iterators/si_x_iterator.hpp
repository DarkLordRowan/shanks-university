...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	si_x_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
	...
	T next(K n, T& state) const override {
		...
		utils::cast<T>(utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1)) * 2 * utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1)) * n);
		return state;
	}
