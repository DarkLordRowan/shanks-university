...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	three_minus_pi_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
     ...
     */
	T get_sum() const override{ return this->x * (utils::cast<T>(3) - utils::cast<T>(std::numbers::pi));}
	...
	T next(K n, T& state) const override {
		...
		utils::cast<T>((n + 1)*(n+2)*utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(3)));
		return state;
	}
