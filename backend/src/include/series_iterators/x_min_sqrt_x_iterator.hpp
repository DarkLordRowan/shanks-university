...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	x_min_sqrt_x_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
	...
	T next(K n, T& state) const override {
		...
		if (n == 0) state = this->x - utils::cast<T>(1);
		else if (n == 1) state = (this->x - utils::cast<T>(1)) * utils::cast<T>(-0.5);
		else state *= utils::cast<T>(-1) * (this->x - utils::cast<T>(1)) * utils::cast<T>(0.5) * utils::cast<T>(2*n - 3) / utils::cast<T>(n);
		return state;
	}
