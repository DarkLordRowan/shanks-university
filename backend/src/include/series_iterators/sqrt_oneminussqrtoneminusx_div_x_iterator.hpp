...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	sqrt_oneminussqrtoneminusx_div_x_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
     ...
     */
	T get_sum() const override{ return utils::sqrt((utils::cast<T>(1) - utils::sqrt(utils::cast<T>(1) - this->x)) / this->x);}
