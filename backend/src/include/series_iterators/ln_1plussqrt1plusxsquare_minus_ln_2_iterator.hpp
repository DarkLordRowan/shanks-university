...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
	ln_1plussqrt1plusxsquare_minus_ln_2_iterator(T x) : series_base_succ<T, K>(x) {
	    if (this->is_invalid())
     ...
     */
	T get_sum() const override{ return utils::log(utils::cast<T>(1)+utils::sqrt(utils::cast<T>(1) + this->x * this->x)) - utils::log(utils::cast<T>(2)); }
	...
	T next(K n, T& state) const override {
		...
		if (n == 0) state = this->x * this->x * utils::cast<T>(0.25);
		else state *= utils::cast<T>(-1) * this->x * this->x *
		utils::cast<T>(utils::fma(static_cast<size_t>(2),static_cast<size_t>(n),static_cast<size_t>(1)) * n) / utils::cast<T>(2 * (n+1)*(n+1));
		return state;
	}
