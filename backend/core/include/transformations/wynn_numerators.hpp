...
public:
     ...
     */
    virtual T operator()(K n, K order, const std::vector<T>& an, T gamma = utils::cast<T>(-1),
...
public:
    ...
    T operator()(const K n, const K order, const std::vector<T>& an, const T gamma = utils::cast<T>(-1),
