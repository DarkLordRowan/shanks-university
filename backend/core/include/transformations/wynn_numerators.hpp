...
namespace shanks
{
    namespace numerators
    {
        ...
        public:
             ...
             */
            virtual T operator()(K n,
                                 K order,
                                 const std::vector<T>& an,
                                 T gamma = utils::cast<T>(-1),
                                 T rho = utils::cast<T>(1)) const = 0;
        };
        ...
        public:
             ...
             *
             * Implements: numerator = term(n + order + 1) - term(n), see p. 34 6.2-2b
             * [https://arxiv.org/pdf/math/0306302] This represents the forward difference of order 'order' at index n.
             *
             * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
            ...
            T operator()(const K n,
                         const K order,
                         const std::vector<T>& an,
                         const T gamma = utils::cast<T>(-1),
                         const T rho = utils::cast<T>(1)) const
            {
        ...
        public:
            ...
            T operator()(const K n,
                         ...
                         const T rho = utils::cast<T>(1)) const
            {
                return utils::cast<T>(order - static_cast<K>(1)) -
                       gamma; // p.377 rhi(gamma)-algorithm
        ...
        public:
            ...
            T operator()(const K n,
                         ...
                         const T rho = utils::cast<T>(1)) const
            {
                // p.377 Automatic rho(gamma)-algorithm
                // [http://servidor.demec.ufpr.br/CFD/bibliografia/MER/Sidi_2003.pdf]

                // For theory, see: Wynn (1962), Section 2: Parameterized transformations
                // Add 1 for odd orders (order & 1 checks parity)
                return -gamma + utils::cast<T>(order) / (utils::cast<T>(2) * rho) +
                       utils::cast<T>(order & static_cast<K>(1));
            }
        };
