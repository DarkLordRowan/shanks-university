...
T shanks_transform_alternating<T, K>::operator()(const K n, const K order, const series_result<T>& data) const {
    ...
    if (data.Sn.size() < required_size || data.an.size() < required_size) {
        throw std::out_of_range("The Sn or an smaller then required for alt_shanks_{" + utils::to_string(order) +
    ...
    if (order == static_cast<K>(1)) [[unlikely]] {
        ...
        result += utils::fma(data.an.at(n) * data.an.at(n + static_cast<K>(1)),
    ...
    std::vector<T> T_n(n + order, utils::cast<T>(0.0, precision));
         ...
         ++i)  // if we got to this branch then we know that n >= order - see previous branches int->K
    {
        // For theory, see: Senhadji (2001), Section 3.2 - Alternating series case
        // e₁(Sᵢ) = Sᵢ + (aᵢaᵢ₊₁)/(aᵢ - aᵢ₊₁)
        T_n[i] = utils::fma(data.an.at(i) * data.an.at(i + static_cast<K>(1)),
                            utils::cast<T>(1, precision) / (data.an.at(i) - data.an.at(i + static_cast<K>(1))),
                            data.Sn.at(n));
    }

    std::vector<T> T_n_plus_1(n + order, utils::cast<T>(0.0, precision));
    ...
    for (K j = static_cast<K>(2); j <= order; ++j) {
        for (K i = n - order + j; i <= n + order - j; ++i) {
            ...
            T_n_plus_1[i] = utils::fma(utils::fma(a, c + b - a, -b * c),
                                       utils::cast<T>(1.0, precision) / (utils::cast<T>(2) * a - b - c), a);
        }
        T_n = T_n_plus_1;
    }
