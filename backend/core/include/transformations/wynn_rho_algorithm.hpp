...
namespace shanks
{
    namespace algos
    {
        ...
        public:
             ...
             */
            void update_rho(const float_type& new_rho)
            {
                rho_in_use = new_rho;
            }
        ...
        inline T wynn_rho_algorithm<T, K>::operator()(const K n, const K order, const series_result<T>& data) const
        {
            ...
            for (K level = static_cast<K>(1); level <= order / static_cast<K>(2); ++level)
            {
                ...
                for (K j = static_cast<K>(0); j < base_size - level; ++j)
                {
                    ...
                    rho_odd[j] = rho_odd[j1] + numerator->operator()(n + j,
                ...
                for (K j = static_cast<K>(0); j < base_size - level; ++j)
                {
                    ...
                    rho_even[j] =
                        rho_even[j1] +
                        numerator->operator()(n + j, level * static_cast<K>(2), data.an, gamma_in_use, rho_in_use) /
                            delta;
                }
            }
