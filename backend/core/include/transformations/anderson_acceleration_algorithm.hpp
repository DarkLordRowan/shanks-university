...
namespace shanks
{
    namespace algos
    {
        ...
        public:
             ...
             * @param order The transformation order (unused in this specific algorithm, kept for interface
             * consistency).
        ...
        inline T anderson_acceleration_algorithm<T, K>::main_case(const K n, const std::vector<T>& Sn) const
        {
            ...
            for (K i = 0; i < actual_m; ++i)
                A[i][i] += utils::cast<T>(safeguard_, precision);
        ...
        template <AcceptedLike T, UnsignedIntLike K>
        T anderson_acceleration_algorithm<T, K>::operator()(const K n,
                                                            const K /*order*/,
