...
public:
    ...
    explicit overholt_algorithm() : series_acceleration<T, K>() {}
...
T overholt_algorithm<T, K>::operator()(const K n, const K order, const series_result<T>& data) const {
    ...
    if (data.Sn.size() < n + order + static_cast<K>(1) && data.an.size() < n + order + static_cast<K>(1)) {
    ...
    std::vector<T> V(n+1, utils::cast<T, int>()(0, precision));
