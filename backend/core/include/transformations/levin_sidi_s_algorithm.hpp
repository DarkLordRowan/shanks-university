...
namespace shanks
{
    namespace algos
    {
        ...
        protected:
             ...
             * (β+n+j)_{k-1}/(β+n+k)_{k-1} S_{n+j}/R_{n+j}] / [∑_{j=0}^k (-1)^j C(k,j) (β+n+j)_{k-1}/(β+n+k)_{k-1}
             * 1/R_{n+j}]
             *
             * @authors Naumov A.U.
             ...
             * ([https://arxiv.org/pdf/math/0306302.pdf]) Recursive implementation for better numerical stability in
             * some cases.
             *
             * @authors Naumov A.U.
             * @param n Starting index for the transformation
        ...
        inline T levin_sidi_s_algorithm<T, K>::calc_result(const K n, const K order, const series_result<T>& data) const
        {
            ...
            for (K j = static_cast<K>(0); j <= order; ++j)
            {
                ...
                // up_pochamer   (beta + n + j)_(order - 1)     = (beta + n + j)(beta + n + j + 1)...(beta + n + j +
                // order - 2) down_pochamer (beta + n + order)_(order - 1) = (beta + n + order)(beta + n + order +
                // 1)...(beta + n + order + oreder - 2)
                ...
                rest *= remainder->operator()(
                    ...
                    n + j,
                    data.an,
                    utils::cast<T>(remainder_type_in_use == shanks::remainders::remainder_type::u_type ? beta_in_use
                                                                                                       : 1.0,
                                   precision));
        ...
        template <AcceptedLike T, UnsignedIntLike K>
        inline T
        levin_sidi_s_algorithm<T, K>::calc_result_rec(const K n, const K order, const series_result<T>& data) const
        {
            const size_t precision = std::max(utils::get_precision(data.Sn[0]), utils::get_precision(data.an[0]));
            ...
            // For theory, see: Sidi (2003, arXiv:math/0306302), pp. 57-58, Eqs. (8.3)-(8.5)
            // ([https://arxiv.org/pdf/math/0306302.pdf]) Recursive implementation for better numerical stability in
            // some cases. Initialization: Weighted terms for order 0 E_0^{(n)} = S_n, g_0^{(n)} = 1/R_n
            ...
            for (K i = static_cast<K>(1); i <= order; ++i)
                ...
                for (K j = static_cast<K>(0); j <= order - i; ++j)
                {
                    ...
                    Denom[j] = utils::fma(utils::cast<T>(-scale1, precision),
                                          Denom[j] / utils::cast<T>(scale2, precision),
                                          Denom[j + static_cast<K>(1)]);
                    Num[j] = utils::fma(utils::cast<T>(-scale1, precision),
                                        Num[j] / utils::cast<T>(scale2, precision),
        ...
        T levin_sidi_s_algorithm<T, K>::operator()(const K n, const K order, const series_result<T>& data) const
        {
            ...
            const K required_size =
                ...
                static_cast<K>(2) *
                    static_cast<K>(remainder_type_in_use == shanks::remainders::remainder_type::v_wave_type);
