...
namespace shanks
{
    namespace algos
    {
        ...
        T wynn_epsilon_3_algorithm<T, K>::operator()(const K n, const K order, const series_result<T>& data) const
        {
            ...
            float_type abs_error = utils::cast<float_type>(0.0,
                                                           precision); ///< Absolute error estimate for current data.
            ...
            for (K k = 0; k <= steps; ++k)
            {
                ...
                // subroutine) Since we are simulating sequential calls, we should likely NOT reset it if we want to
                // track the *best* result found so far? However, the Fortran routine outputs RESULT and ABSERR. If we
                // treat this loop as "finding the best result within the window", we should maintain 'result' and
                // 'abs_error' across iterations. BUT, the Fortran code: "ABSERR = OFRN" is unconditional at the start.
                ...
                if (shift_logic)
                {
                    ...
                    if (k != num_k)
                    { // IF (NUM.EQ.N) check (k is original N, num_k is potentially modified N)
                        // Fortran: IN = NUM - N + 1.
