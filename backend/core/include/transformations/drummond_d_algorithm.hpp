...
namespace shanks
{
    namespace algos
    {
        ...
        public:
             ...
             * @authors Naumov A.U.
             */
            explicit drummond_d_algorithm(const shanks::remainders::remainder_type remainder_type_to_use =
                                              shanks::remainders::remainder_type::u_type,
        ...
        template <AcceptedLike T, UnsignedIntLike K>
        inline T
        drummond_d_algorithm<T, K>::calc_result_rec(const K n, const K order, const series_result<T>& data) const
        {
            const size_t precision = std::max(utils::get_precision(data.Sn[0]), utils::get_precision(data.an[0]));
