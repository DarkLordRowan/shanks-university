...
class rump_seq2_iterator final : public series_base_iter<T, K, std::pair<T, T>> {
public:
    /**
     * @brief Default constructor for rump_seq2_iterator.
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
    explicit rump_seq2_iterator(T x) : series_base_iter<T, K, std::pair<T, T>>(x) {
    ...
    T next(K n, std::pair<T, T>& state) const override {
        ...
        if (n == 0)
            state = std::make_pair(utils::cast<T, int>()(0, precision), utils::cast<T, int>()(3, precision));
        else if (n == 1)
            state = std::make_pair(utils::cast<T, int>()(3, precision), utils::cast<T, int>()(170, precision));
        else {
