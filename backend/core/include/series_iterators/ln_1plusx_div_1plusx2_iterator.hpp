...
public:
     ...
     */
	T get_sum() const override{ return utils::log(utils::cast<T>(1) + this->x) / (utils::cast<T>(1) + this->x * this->x); }
	...
	T next(K n, T& state) const override {
		...
		if (n == 0) state = this->x  / (utils::cast<T>(1) + this->x * this->x);
		else state *= utils::cast<T>(-1) * this->x * utils::cast<T>(n) / (utils::cast<T>(n+1));
		return state;
	}
