...
namespace shanks
{
    namespace series
    {
        ...
        public:
            ...
            {
                return utils::log(utils::cos(this->x));
            }
            ...
            bool is_invalid() const override
            {
                ...
                return !utils::isfinite(this->x) ||
                       utils::abs(this->x) >= utils::cast<float_type>(std::numbers::pi * 0.5);
            }
            ...
            T next(K n, T& state) const override
            {
                ...
                if (n == 0)
                    state = utils::sin(this->x) * utils::sin(this->x) * utils::cast<T>(-0.5);
                else
                    state *= utils::sin(this->x) * utils::sin(this->x) * utils::cast<T>(n) / utils::cast<T>(n + 1);
                return state;
            }
        };
