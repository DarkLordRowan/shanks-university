...
public:
     ...
     */
    T get_sum() const override { return this->x * utils::cast<T>::meta(std::numbers::pi * 5) / utils::cast<T>::meta(12); }
    ...
    T next(K n, T& state) const override {
        ...
        if (n == 0)
            state = utils::cast<T>::meta(5) * this->x / utils::cast<T>::meta(3);
        else
            state = utils::cast<T>::meta(5) / utils::cast<T>::meta(3) * utils::minus_one_raised_to_power_n<T, K>(n) * this->x /
