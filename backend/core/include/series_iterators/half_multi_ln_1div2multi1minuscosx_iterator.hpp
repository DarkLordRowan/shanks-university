...
public:
    ...
    T get_sum() const override {
        return utils::cast<T>::meta(-0.5) * utils::log(utils::cast<T>::meta(2) - utils::cast<T>::meta(2) * utils::math<T>::cos(this->x));
    }
    ...
    T next(K n, T& state) const override {
        ...
        state = utils::math<T>::cos(utils::cast<T>::meta(n + 1) * this->x) / utils::cast<T>::meta(n + 1);
