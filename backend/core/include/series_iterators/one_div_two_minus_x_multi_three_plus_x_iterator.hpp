...
public:
	...
	T next(K n, T& state) const override {
		...
		state = utils::pow(this->x, utils::cast<T>(n)) *
		utils::cast<T>(0.2) * (utils::minus_one_raised_to_power_n<T, K>(n) * utils::cast<T>(utils::pow(static_cast<size_t>(2), static_cast<size_t>(n) + 1)) +
		utils::cast<T>(utils::pow(static_cast<size_t>(3), static_cast<size_t>(n) + 1))) / utils::cast<T>(utils::pow(static_cast<size_t>(6), static_cast<size_t>(n)+1));
