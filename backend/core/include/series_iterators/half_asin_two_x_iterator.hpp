...
namespace shanks
{
    namespace series
    {
        ...
        public:
            ...
            {
                return utils::cast<T>(0.5) * utils::asin(utils::cast<T>(2) * this->x);
            }
            ...
            {
                // Recurrence relation derived from the arcsin expansion with substitution x -> 2x and scaling factor
                // 0.5
                if (n == 0)
                    state = this->x;
                else
                    state *=
                        ...
                        utils::cast<T>(
                            ...
                            utils::fma(static_cast<size_t>(2), static_cast<size_t>(n - 1), static_cast<size_t>(1))) /
                        utils::cast<T>(
                            n * n * utils::fma(static_cast<size_t>(2), static_cast<size_t>(n), static_cast<size_t>(1)));
                return state;
            }
        };
