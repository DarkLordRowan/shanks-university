...
public:
     ...
     */
    T get_sum() const override { return utils::cast<T, int>()(4) * utils::math<T>::atan(this->x); }
    ...
    T next(K n, T& state) const override {
        ...
        if (n == 0)
            state = utils::cast<T, int>()(4) * this->x;
        else
            state *= utils::cast<T, int>()(-1) * this->x * this->x * utils::cast<T, K>()(2 * n - 1) /
