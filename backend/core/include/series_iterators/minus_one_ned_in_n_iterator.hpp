...
class minus_one_ned_in_n_iterator final : public series_base_succ<T, K> {
public:
    /**
     * @brief Default constructor for minus_one_ned_in_n_iterator.
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
    explicit minus_one_ned_in_n_iterator(T x) : series_base_succ<T, K>(x) {
     ...
     */
    T get_sum() const override { return this->x * utils::cast<T, double>()(-0.78343051); }
    ...
    T next(K n, T& state) const override {
        ...
        state =
            utils::math<T>::template minus_one_raised_to_power_n<K>(n + 1) * this->x / utils::cast<T, K>()(utils::math<K>::pow(n + 1, n + 1));
        return state;
    }
};
