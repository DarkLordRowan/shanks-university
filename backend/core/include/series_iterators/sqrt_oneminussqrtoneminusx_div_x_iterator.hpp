...
public:
    ...
    T get_sum() const override {
        return utils::math<T>::sqrt((utils::cast<T>::meta(1) - utils::math<T>::sqrt(utils::cast<T>::meta(1) - this->x)) / this->x);
    }
    ...
    bool is_invalid() const override {
        ...
        return !utils::helpers<T>::isfinite(this->x) || this->x == utils::cast<T>::meta(0) ||
               utils::math<T>::abs(this->x) >= utils::cast<float_type>(1);
    }
    ...
    T next(K n, T& state) const override {
        ...
        if (n == 0)
            state = utils::cast<T>::meta(1, utils::helpers<T>::get_precision(state)) /
                    utils::math<T>::sqrt(utils::cast<T>::meta(2, utils::helpers<T>::get_precision(state)));
        else
            state *= this->x *
