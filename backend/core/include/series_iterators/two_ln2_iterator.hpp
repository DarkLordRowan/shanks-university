...
public:
     ...
     */
    T get_sum() const override { return this->x * utils::math<T>::log(utils::cast<T>::meta(4)); }
    ...
    T next(K n, T& state) const override {
        ...
        state = utils::cast<T>::meta(12 * n1 * n1 - 1) * this->x / utils::cast<T>::meta(n1 * (4 * n1 * n1 - 1) * (4 * n1 * n1 - 1));
        return state;
    }
};
