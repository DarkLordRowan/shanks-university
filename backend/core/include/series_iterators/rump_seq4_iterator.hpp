...
class rump_seq4_iterator final : public series_base_seq_iter<T, K, std::pair<T, T>> {
public:
    /**
     * @brief Default constructor for rump_seq4_iterator.
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
    explicit rump_seq4_iterator(T x) : series_base_seq_iter<T, K, std::pair<T, T>>(x) {
    ...
    T next(K n, std::pair<T, T>& state) const override {
        ...
        if (n == 0)
            state = std::make_pair(
                ...
                this->x + utils::cast<T, int>()(109225, precision) / utils::cast<T, int>()(43691.0, precision));
        else if (n == 1)
            state = std::make_pair(
                this->x + utils::cast<T, int>()(109225, precision) / utils::cast<T, int>()(43691.0, precision),
