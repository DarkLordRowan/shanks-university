#ifndef RUMP_SEQ6_ITERATOR_HPP
#define RUMP_SEQ6_ITERATOR_HPP
#pragma once

#include "../series_base.hpp"

/**
 * @file rump_seq6_iterator.hpp
...
class rump_seq6_iterator final : public series_base_seq_iter<T, K, std::pair<T, T>> {
public:
    /**
     * @brief Default constructor for rump_seq6_iterator.
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
    explicit rump_seq6_iterator(T x) : series_base_seq_iter<T, K, std::pair<T, T>>(x) {
        if (this->is_invalid()) throw std::invalid_argument("Invalid series argument");
    }

    /**
     * @brief Retrieves limit for the series x_{n+1} = 82 - (1824 - 6048 / x_{n-1})/x_{n}, x_0 = -6, x_1 = 64
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     * @return T The limit of the sequence.
     */
    T get_sum() const override { return utils::cast<T, int>()(36); }
    ...
    T next(K n, std::pair<T, T>& state) const override {
        ...
        if (n == 0)
            state = std::make_pair(utils::cast<T, int>()(0, precision), this->x + utils::cast<T, int>()(-6, precision));
        else if (n == 1)
            state =
                std::make_pair(this->x + utils::cast<T, int>()(-6, precision), utils::cast<T, int>()(64, precision));
        else {
            state.first =
