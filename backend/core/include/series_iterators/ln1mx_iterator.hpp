...
public:
     ...
     * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
     */
    explicit ln1mx_iterator(T x) : series_base_succ<T, K>(x) {
     ...
     */
    T get_sum() const override { return utils::cast<T>::meta(-1) * utils::log(utils::cast<T>::meta(1) - this->x); }
    ...
    T next(K n, T& state) const override {
        ...
        if (n == 0)
            ...
            state *= this->x * utils::cast<T>::meta(n) / utils::cast<T>::meta(n + 1);
        return state;
    }
};
