...
template<typename T>
struct GetUnderlyingType<intprec::interval<T>>{using value = T; };

template<typename T>
struct real_of { using value = T; };

template<FloatLike T>
struct real_of<std::complex<T>> {using value = T;};
...
struct utils {
	 ...
	 */
	template <AcceptedLike T, UnsignedIntLike K> static T phi(K n);
	 ...
	 */
	template <typename K>
	requires UnsignedIntLike<K> || is_standard_types<K>::value static K fact(const K n);
	 ...
	 */
	template <UnsignedIntLike K> static K double_fact(const K n);
	 ...
	 */
	template <UnsignedIntLike K> static K binomial_coefficient(const K n, const K k);
	 ...
	 */
	template<AcceptedLike T, UnsignedIntLike K> static T minus_one_raised_to_power_n(const K j);
	...
	template<AcceptedLike T> static T log10(const T& x);
