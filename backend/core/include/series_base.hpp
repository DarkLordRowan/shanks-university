...
protected:
    ...
    static series_result<T> generate_from_iterator(K n, IteratorFunc func, bool is_seq = false) {
        ...
        std::vector<T> an(n, utils::cast<T>::meta(0.0, utils::helpers<T>::get_precision(sample)));
        std::vector<T> Sn(n, utils::cast<T>::meta(0.0, utils::helpers<T>::get_precision(sample)));

        an[0] += sample;
        Sn[0] += sample;
        for (K i = 1; i < n; ++i) {
            an[i] += func();
            Sn[i] += an[i] + (is_seq ? utils::cast<T>::meta(0.0) : Sn[i - static_cast<K>(1)]);
        }
...
public:
    ...
    virtual T get_sum() const override = 0;
    T initial_state() const { return utils::cast<T>::meta(0.0, utils::helpers<T>::get_precision(this->x)); };
