 ...
 * -------  -----------		----------------------
 * 03.01	HVE/3-Oct-2021	Switch to from Decimal String to Binary fptype float numbers. The previous chnage record
 * has been removed to simplify 03.02	HVE/19-Oct-2021	Passed all float testing 03.03	HVE/19-Nov-2021 Fixed complier
 * bugs reported n GNU version 14 running on a Mac 03.04	HVE/20-Nov-2021 A few bugs fixed and change to avoid
 * compiler warnings 03.05	HVE/22-Nov-2021	Minor change and improvement 03.06	HVE/2-Dec-2021	Fix a bug in
 * _float_precision_left_shift() that did not shift correctly if shift count was >= 64 03.07	HVE/8-Dec-2021	Change
 * eptype to an intmax_t insead of int to raise the exponent limit to more than 300M digits 03.08	HVE/10-Dec-2021
 *	Added two more trunking levels for faster handling of digits exceedig 10-100M decimal digits 03.09
 *	HVE/11-Dec-2021	atofp() can be called with a std::string or a char * 03.10	HVE/25-Dec-2021 Added
 * _float_precision_schonhage_strassen_linear_umul() to add better multiplication for medium size multiplication of
 * digits<6,000 the function was modified from the int_precision counterpart 03.11	HVE/6-Jan-2022	Added _INVSQRT3
 * and SQRT3 as a build in constant 03.12	HVE/4-Feb-2022	Improved the precision() to avoid call rounding when
 * precision is increased 03.13	HVE/23-Mar-2022	Cleaning up the code 03.14	HVE/23-Apr-2022 Added the method
 * .square() and .inverse for fast squaring of a float_precision number or inversion of a number 03.15	HVE/21-May-2022
 * Added AGM() & log2() using float_precision parameters (Arithmetic-Geometric Mean) 03.16	HVE/7-Aug-2022	Handle a
 * carry bug in the _int_precision_umul_add() 03.17	HVE/20-Sep-2022	Added a extra default parameter to function
 * _float_precision_clz() 03.18	HVE/29-Oct-2022	Added the succ and pred method for finding next or previous
 * representable number (usefull for interval arithmetic) 03.19	HVE/15-Nov-2022	Fixed an issue with construction form a
 * double. For constant 0, 1 or 2^x the mantissa could have one extra zero element 03.20	HVE/20-Dec-2022	Added
 * Bernoulli and Bernoulli Polynomials functions declarations 03.21	HVE/1-Dec-2023	Added the _EULER
 * (Euler-Mascheroni) constant as a parameter to the float_table() function 03.22	HVE/10-Jan-2023	Added the
 * _CATALAN (Catalan constant) as a paramter to the float_table() function 03.23	HVE/10-Jan-2023	Added the
 * tgamma() declaration 03.24	HVE/13-Jan-2023	Added the beta(), erf() and erfc() declaration 03.25	HVE/23-Jan-2024
 *	Added the _ZETA3 (Apery constant) as a parameter to the float_table() function 03.26	HVE/7-Mar-2024	Added
 * the declaration of fma() 03.27	HVE/15-Mar-2024	Fixed a bug in the nextafter succ() and pred() function when
 * argument was zero 03.28	HVE/23-Mar-2024	Move some static const only used in precisioncore.cpp to
 * precisioncore.cpp 03.29	HVE/29-Mar-2024	An declaration error and a bug in frexp() was corrected 03.30
 *	HVE/27-Nov-2024	Added stringsteam support for the cin/cout >> operator
 *
...
[[maybe_unused]] static char _VF_[] = "@(#)fprecision.h 03.30 -- Copyright (C) Henrik Vestermark";
...
//[CUSTOM] narrow down types
template <typename T>
struct is_supported_types : public std::false_type {};

template <std::floating_point T>
struct is_supported_types<T> : public std::true_type {};

template <std::integral T>
struct is_supported_types<T> : public std::true_type {};

template <typename T>
concept supported_types = is_supported_types<T>::value;

///[CUSTOM] comparisons
template <supported_types _Ty>
...
const unsigned int Bitsfptype =
    sizeof(fptype) * 8;  // Const use throughtout the source which is the number of bits the fptype can hold.
...
template <supported_types _Ty>
inline float_precision operator+(const float_precision&, const _Ty&);
template <supported_types _Ty>
inline float_precision operator+(const _Ty&, const float_precision&);
inline float_precision operator+(const int_precision&,
                                 const float_precision&);  // Override int_precision - other type in iprecision.h
inline float_precision operator+(const float_precision&, const float_precision&);
...
extern float_precision fabs(
    const float_precision&);  // Obsolete. replaced by overloaded abs(). But here for backward compatitbility
...
std::vector<fptype> _float_precision_umulsq_linear(
    const std::vector<fptype>&);  //  Squaring number using linear convolution
...
class float_precision {
    ...
    size_t mPrec;  // Number of decimals in mantissa. Default 20, We make a shot cut by assuming the number of digits
                   // can't exceed 2^32-1 on 32bit or 2^64-1 on 64bit system
    eptype mExpo;  // Exponent as a power of 2 as in IEEE 754. We make a short cut here and use the eptype to hold the
                   // exponent the exponent.
    int mSign;     // The sign +1 for "+"a and -1 for "-". Notice in version 2+ the sign has been separated frm the
                   // mNumber, same as for int_precision
    std::vector<fptype> mNumber;  // The binary vector of fptype that holds the float number. Per definition the vector
                                  // when the constructor is invoked will always be initialized to zero if no argument
                                  // is provided. The fraction point is always after the first digit and is implied.
                                  // mNumber[0] holds the most significant part of the number. e.g. R=2^64.
                                  // Number=mNumber[0]*R^0+mNumber[1]*R^-1+mBInary[2]*R^-2,...mNumber[n-1]*R^-(n-1) etc.
...
public:
    ...
    float_precision(const std::vector<fptype>& v);  // When initialized through a std::vector<iptype>. Notice sign will
                                                    // be 1 since vector<iptype> is unsigned
    float_precision(const float_precision& s);      // When initialized through another float_precision
    float_precision(const int_precision&, const size_t,
                    const enum round_mode);  // When initialized through a int_precision
    ...
    float_precision
    pred();  // Return the previous representable value of the number towards negative infinity
...
//
inline float_precision::float_precision(const char c, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    mExpo = 0;
    mRmode = m;
    mPrec = p;
    mSign = +1;
    ...
    mNumber.assign(1, std::abs(c));
    if (c != 0) mExpo += _float_precision_normalize(mNumber);
}
...
//  Validate and initilize with a character
//
inline float_precision::float_precision(const unsigned char c, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    mExpo = 0;
    mRmode = m;
    mPrec = p;
...
//
inline float_precision::float_precision(short i, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    mExpo = 0;
    mRmode = m;
    mPrec = p;
    ...
    if (i != 0) mExpo += _float_precision_normalize(mNumber);
}
...
//  Validate and initialize with integer
//
inline float_precision::float_precision(unsigned short i, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    mExpo = 0;
    mRmode = m;
    mPrec = p;
...
//  Validate and initialize with integer
//
inline float_precision::float_precision(int i, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    mExpo = 0;
    mRmode = m;
    mPrec = p;
...
//  Validate and initialize with integer
//
inline float_precision::float_precision(unsigned int i, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    mExpo = 0;
    mRmode = m;
    mPrec = p;
...
//  Validate and initialize with integer
//
inline float_precision::float_precision(long i, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    mExpo = 0;
    mRmode = m;
    mPrec = p;
...
//  Validate and initialize with integer
//
inline float_precision::float_precision(unsigned long i, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    mExpo = 0;
    mRmode = m;
    mPrec = p;
...
//  Validate and initialize with integer
//
inline float_precision::float_precision(long long i, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    mExpo = 0;
    mRmode = m;
    mPrec = p;
...
//  Validate and initialize with integer
//
inline float_precision::float_precision(unsigned long long i, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    mExpo = 0;
    mRmode = m;
    mPrec = p;
...
//
inline float_precision::float_precision(const char* str, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    if (str == NULL || *str == '\0') {
...
inline float_precision::float_precision(const std::string& str, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    if (str.empty()) {
        throw bad_int_syntax();
        return;
...
//
inline float_precision::float_precision(
    float f, const size_t p = float_precision_ctrl.precision(),
    const enum round_mode m = float_precision_ctrl.mode()) {  // Call the constructor for double
    mPrec = p;
    mRmode = m;
...
//  The float can be any integer or decimal float representation
//
inline float_precision::float_precision(double d, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    uintmax_t fpb;
    ...
    if (fpb != 0) mNumber.push_back(fpb);  // The mantissa without the 1
}
...
//
inline float_precision::float_precision(const int_precision& ip, const size_t p = float_precision_ctrl.precision(),
                                        const enum round_mode m = float_precision_ctrl.mode()) {
    std::string s;
    const std::vector<iptype> inumber(ip.number());

    mSign = ip.sign();
    for (size_t i = inumber.size(); i > 0; --i) mNumber.push_back((fptype)inumber[i - 1]);
    mRmode = m;
    mPrec = p;
...
inline float_precision::float_precision(unsigned long i, const size_t p, const enum round_mode m, const int sign,
                                        const eptype expo) {
    mExpo = expo;
    mRmode = m;
    mPrec = p;
...
//
inline float_precision::operator double() const {  // Conversion to double
    uintmax_t t = 0, expo;
    double d;

    if (this->size() == 1 && this->index(0) == 0) return 0.0;
    expo = this->exponent();  // Get the exponent
    expo += 1023;             // Add Biased double exponent format
    if (this->size() > 1) t = static_cast<uintmax_t>(this->index(1));
    t >>= 12;
...
//
inline float_precision::operator float() const {  // Conversion to float
...
//
inline float_precision::operator long long() const {  // Conversion to long long
...
//
inline float_precision::operator long() const {  // Conversion to long
...
//
inline float_precision::operator int() const {  // Conversion to int
...
//
inline float_precision::operator short() const {  // Conversion to short
...
//
inline float_precision::operator char() const {  // Conversion to char
...
//
inline float_precision::operator unsigned long long() const {  // Conversion to unsigned long long
    ...
    if (mExpo == 0) {
        return ull = static_cast<unsigned long long>(mNumber[0]);
...
//
inline float_precision::operator unsigned long() const {  // Conversion to unsigned long
...
//
inline float_precision::operator unsigned int() const {  // Conversion to unsigned int
...
//
inline float_precision::operator unsigned short() const {  // Conversion to unsigned short
...
//
inline float_precision::operator unsigned char() const {  // Conversion to unsigned char
    return static_cast<unsigned char>(static_cast<unsigned long long>(*this));
}

inline float_precision::operator signed char() const {  // Conversion to unsigned char
    return static_cast<signed char>(static_cast<signed long long>(*this));
}
...
//
inline float_precision::operator int_precision() const {  // Conversion to int_precision
...
//
inline enum round_mode float_precision::mode() const { return mRmode; }
...
//
inline enum round_mode float_precision::mode(const enum round_mode m) { return (mRmode = m); }
...
//
inline eptype float_precision::exponent() const { return mExpo; }
...
//
inline eptype float_precision::exponent(const eptype e) { return (mExpo = e); }
...
//	Much faster way to multiply with anu power of two. If mNumber[0] is 0 then dont do any adjustment since the
// result is still 0
//
inline eptype float_precision::adjustExponent(const eptype adj) {
    if (mNumber[0] != 0) mExpo += adj;
    return mExpo;
}
...
//
inline int float_precision::sign() const { return mSign; }
...
//
inline int float_precision::sign(const int s) { return (mSign = s); }
...
//
inline int float_precision::change_sign() { return (mSign *= -1); }
...
//
inline size_t float_precision::precision() const { return mPrec; }
...
inline size_t float_precision::precision(const size_t p) {
    ...
    if (newPrec < mPrec) mExpo += _float_precision_rounding(mNumber, mSign, newPrec, mRmode);
    mPrec = newPrec;
    return mPrec;
}
...
//
inline std::vector<fptype> float_precision::number() const { return mNumber; }
...
//
inline std::vector<fptype> float_precision::number(const std::vector<fptype>& mb) { return mNumber = mb; }
...
//
inline std::vector<fptype>* float_precision::pointer() { return &mNumber; }
...
//
inline fptype float_precision::index(const size_t inx) const { return mNumber[inx]; }
...
//
inline size_t float_precision::size() const { return mNumber.size(); }
...
inline bool float_precision::iszero() const {
    return mNumber.size() == 1 && mNumber[0] == 0;
}  // Notice both +0 and -0 is allowed and return true
...
inline float_precision float_precision::epsilon() {
    ...
    res.exponent(1 - (eptype)(std::ceil(mPrec * std::log2(BASE_10))));  // Number of precision bits needed
    return res;
}
...
inline float_precision float_precision::succ() {
    ...
    const intmax_t pbits =
        static_cast<intmax_t>(std::ceil(mPrec * std::log2(BASE_10)));  // +1;// -1; // Number of precision in bits
...
inline float_precision float_precision::pred() {
    float_precision next(*this);  // Copy the original number
    const intmax_t pbits =
        static_cast<intmax_t>(std::ceil(mPrec * std::log2(BASE_10)));  // +1; // Number of precision in bits
    float_precision one(1);
    eptype expo;
...
//
inline float_precision float_precision::inverse() { return _float_precision_inverse(*this); }
...
inline float_precision float_precision::square() {
    ...
    if (mNumber[0] == 0)  // zero can only arise if the number is zero and therefore no need to check the size
        s.assign(1, 0);
    else {  // Both mNumber[0] == 1 check size to dermine if it is 1 or any power of 2
        if (mNumber.size() > 1) {
...
//	@author Henrik Vestermark (hve@hvks.com)
//	@date		22/Nov/2021
//	@brief 		float_precision::toInteger()  reduce float precision to a integer and return the discared
// fraction part
...
//
inline float_precision float_precision::toInteger() {  // Trunck towards zero the floating number to its integer part
                                                       // and return the remaing fraction part
    ...
    if (mNumber.size() > n + extra) mNumber.erase(mNumber.begin() + n + extra, mNumber.end());
    // Discard excessive bits within the last fptype.
    if (n < mNumber.size()) mNumber[n] &= ((~static_cast<fptype>(0)) << (Bitsfptype - bn));

    // Adjust the fraction part
    frac.mNumber = _float_precision_left_shift(frac.mNumber, frac.mExpo);
    if (frac.mNumber.size() == 0)
        frac.mNumber.push_back(0);
    else
        frac.mNumber[0] = 0;
    frac.mExpo = _float_precision_normalize(frac.mNumber);
...
//	@author Henrik Vestermark (hve@hvks.com)
//	@date		22/Nov/2021
//	@brief 		float_precision::toFraction  reduce float precision to a fraction and return the discared
// integer part
...
//
inline float_precision
float_precision::toFraction() {  // Return the integer as a float_precision and discard the inteer portion on this
    const fptype mask = ~static_cast<fptype>(0);
    float_precision integer = *this;

    if (mExpo < 0) {
        integer.mNumber.assign(1, 0);
        integer.mExpo = 0;
        integer.mSign = 1;
        return integer;
    }
    if (mExpo == 0) {
        integer.mNumber.assign(1, mNumber[0]);
        mNumber[0] = 0;
        mExpo += _float_precision_normalize(mNumber);
        return integer;
    }
    ...
    // Discard excessive bits within the last fptype.
    if (n < integer.mNumber.size()) integer.mNumber[n] &= (mask << (Bitsfptype - bn));
...
inline std::string float_precision::toString() const {
    if (::arb::isnan(*this)) return std::string("NaN");
    if (::arb::isinf(*this)) return std::string(this->mSign > 0 ? "infinity" : "-infinity");
...
inline std::string float_precision::toFixed(int fix = 0) {
    ...
    sign = mSign;                          // get sign
    if (sign < 0) ss.erase(0, 1);          // Erase sign
    ...
    if (expo > 0) {
        ...
        ss.insert(0, -expo, '0');                                   // Padd with leading zeros
    _float_precision_rounding(&ss, sign, inx + fix, this->mRmode);  // Round to fix. NOTICE string rounding
    if (ss.length() < (unsigned)(inx + fix))
        ss.append(inx + fix - ss.length(),
                  '0');  // ss.insert(ss.length(), inx + fix - ss.length(), '0');		// Add trailing zeros
...
inline std::string float_precision::toPrecision(int fix = 1) {
    ...
    if (expo >= 0) {
        ...
        {
            shf = ss.length() - inx;
            if (shf > static_cast<unsigned>(expo)) shf = expo;
            expo -= static_cast<int>(shf);
            inx += shf;
        }
    } else {  // Expo < 0
        if (expo < 0 &&
            (unsigned)fix > ss.length())  // Room for adding leading zeros to accomodate the exponen which is negative
        {
    ...
    if (expo != 0) {
        ss += "E";
        ss += (expo < 0 ? "-" : "");
        ss += itostring(std::abs(expo), BASE_10);
    }
    return ss;
}
...
inline std::string float_precision::toExponential(int fix = 0) {
    ...
    if ((unsigned)fix > ss.length())
        ss.append(
            fix - ss.length(),
            '0');  // ss.insert(ss.length(), fix - ss.length(), '0');  // Trailing with zeros, so we have fix decimals
    ss = ss.substr(0, inx) + ((unsigned)fix > inx ? "." : "") + ss.substr(inx, fix);
    if (sign < 0) ss.insert(0, 1, '-');  // Add sign if negative
    if (expo != 0) {
        ss += "E";
...
inline float_precision& float_precision::operator+=(const float_precision& rhs) {
    ...
    if (::arb::isnan(*this)) return *this;
    if (::arb::isnan(rhs)) return *this = rhs;

    // Check for shortcuts involving infinity operand
    if (::arb::isinf(rhs) || ::arb::isinf(*this)) {      // One or both the operand is infinity
        if (::arb::isinf(rhs) && ::arb::isinf(*this)) {  // Both operand is of infinity value
        ...
        if (::arb::isinf(*this)) return *this;  // inf += b == inf
    ...
    if (rhs.mExpo != expo_max) s1 = _float_precision_right_shift(s1, expo_max - rhs.mExpo);
    if (mExpo != expo_max) s2 = _float_precision_right_shift(s2, expo_max - mExpo);
    // Now s1 and s2 is aligned to the same exponent. The biggest of the two

    // Round to same precision
    if (_float_precision_rounding(s1, sign1, precision_max, rhs.mode()) !=
        0)  // If carry when rounding up then one right shift
        _float_precision_right_shift(s1, 1);
    if (_float_precision_rounding(s2, sign2, precision_max, mRmode) !=
        0)  // If carry when rounding up then one right shift
...
inline float_precision& float_precision::operator*=(const float_precision& rhs) {
    ...
    if (::arb::isinf(rhs) || ::arb::isinf(*this)) {      // One or both the operand is infinity
        ...
        if (::arb::isinf(*this))  // lhs infinity?
        {
            mSign *= rhs.mSign;
...
inline float_precision& float_precision::operator/=(const float_precision& rhs) {
    ...
    if (::arb::isinf(rhs) || ::arb::isinf(*this)) {  // One or both the operand is infinity
        std::vector<fptype> s;
        if (::arb::isinf(rhs) && ::arb::isinf(*this)) {  // Both operand is of infinity value then return NaN.
            // But maintain precision
            return *this = FP_QUIET_NAN;
        }
        if (::arb::isinf(*this)) {  // Left hand side infinity
            mSign = rhs.mSign;
    ...
    if (rhs.precision() < mPrec) c.precision(mPrec);
    c = rhs;
    *this *= c.inverse();
    return *this;
}
...
//
template <supported_types _Ty>
inline float_precision operator+(const float_precision& lhs, const _Ty& rhs) {
    float_precision c(rhs);
...
//
inline float_precision operator+(const int_precision& lhs, const float_precision& rhs) {
    float_precision c(lhs);
...
inline float_precision operator+(const float_precision& lhs, const float_precision& rhs) {
    float_precision c(lhs);

    c.precision(std::max(lhs.precision(), rhs.precision()));
    return c += rhs;
}
...
inline bool operator==(const float_precision& a, const float_precision& b) {
    ...
    if (a.exponent() != b.exponent())  // exponent differes then return false
        return false;
    if (_float_precision_compare(a.mNumber, b.mNumber) != 0) return false;
    return true;
}
...
inline bool operator<(const float_precision& a, const float_precision& b) {
    ...
    if (a.exponent() < b.exponent()) return sign1 > 0 ? true : false;
    if (a.exponent() > b.exponent()) return sign1 > 0 ? false : true;

    // Same sign & same exponent. Check mantissa
    cmp = _float_precision_compare(
        a.mNumber, b.mNumber /*const_cast<float_precision&>(a).number(), const_cast<float_precision&>(b).number()*/);
    if (cmp < 0 && sign1 == 1) return true;
    if (cmp > 0 && sign1 == -1) return true;
    return false;
}
...
//
inline bool operator!=(const float_precision& a, const float_precision& b) { return !(a == b); }
...
//
inline bool operator>(const float_precision& a, const float_precision& b) { return b < a; }
...
template <supported_types _Ty>
inline bool operator>(const float_precision& a, const _Ty& b) {
    const float_precision c(b, std::max(a.precision(), float_precision(0).precision()));
    return a > c;
}
template <supported_types _Ty>
inline bool operator>(const _Ty& a, const float_precision& b) {
    const float_precision c(a, std::max(b.precision(), float_precision(0).precision()));
    return c > b;
}
template <supported_types _Ty>
inline bool operator<(const float_precision& a, const _Ty& b) {
...
//  return the absolute value of the float_precision number
//
inline float_precision fabs(const float_precision& a) { return ::arb::abs(a); }
