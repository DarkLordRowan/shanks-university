...
namespace intprec {
	...
	template<FloatLike IT> inline IT infinity_interval();
	...
	template<> inline mpfr::mpreal infinity_interval<mpfr::mpreal>() { return mpfr::const_infinity(); }

	// Underflow declaration for the various types
	template<FloatLike T> inline T underflow_interval();
	...
	template<> inline long double underflow_interval<long double>() { return LDBL_MIN; }
	template<> inline mpfr::mpreal underflow_interval<mpfr::mpreal>() { return mpfr::minval(); }
	...
	std::pair<IT, IT> interval<IT>::two_prod(const IT& a, const IT& b)
	{
		...
		const IT t1(utils::abs(p)>ldexp(1,-1021)? (-p*0.5+(x.first*0.5)*y.first)*2.0 : -p+x.first*y.first);
	...
	template<FloatLike IT> inline  interval<IT>::interval() 
	...
	template<FloatLike IT> inline  interval<IT>::interval(const IT& d)
		: left(d), right(d), type(CLOSE), decoration(COM) {}	// Default is closed type

	// Construct a regular interval
	// Since IT is either float or double and the argument is of the same type 
	// we can't catch any conversion error for up or down conversion of the argument
	template<FloatLike IT> inline interval<IT>::interval(const IT& l, const IT& h, const enum interval_type t)
	...
	template<FloatLike IT> template<typename _X> inline interval<IT>::interval(const _X& x)  
	{
	...
	template<FloatLike IT> template<typename _X, typename _Y> inline interval<IT>::interval(const _X& x, const _Y& y, const enum interval_type t)
	{
	...
	template<FloatLike IT> template<typename _X> inline interval<IT>::interval(const interval<_X>& a) //
	{
	...
	template<FloatLike IT> inline interval<IT>::operator short() const
	{	// Conversion to short 	
		return static_cast<short>(mid());
	}
	template<FloatLike IT> inline interval<IT>::operator int() const
	{	// Conversion to int 	
		return static_cast<int>(mid());
	}

	template<FloatLike IT> inline interval<IT>::operator long() const
	{	// Conversion to long 	
		return static_cast<long>(mid());
	}
	template<FloatLike IT> inline interval<IT>::operator long long() const
	{	// Conversion to long long 	
		return static_cast<long long>(mid());
	}

	template<FloatLike IT> inline interval<IT>::operator unsigned short() const
	{	// Conversion to unsigned short 	
		return static_cast<unsigned short>(mid());
	}
	template<FloatLike IT> inline interval<IT>::operator unsigned int() const
	{	// Conversion to unsgined int 	
		return static_cast<unsigned int>(mid());
	}

	template<FloatLike IT> inline interval<IT>::operator unsigned long() const
	{	// Conversion to unsigned long 	
		return static_cast<unsigned long>(mid());
	}

	template<FloatLike IT> inline interval<IT>::operator unsigned long long() const
	{	// Conversion to long long 	
		return static_cast<unsigned long long>(mid());
	}

	template<FloatLike IT> inline interval<IT>::operator double() const
	{	// Conversion to double
		return static_cast<double>(mid());
	}
	template<FloatLike IT> inline interval<IT>::operator float() const
	{	// Conversion to float
	...
	template<FloatLike IT> inline IT interval<IT>::rightinterval() const { return right; }

	// Return the left interval "as is"
	template<FloatLike IT> inline IT interval<IT>::leftinterval() const { return left; }

	// Set the right interval "as is". 
	// If interval is empty set the interval type to CLOSE
	template<FloatLike IT> inline IT interval<IT>::rightinterval(const IT& r) 
	{ 
		if (type == EMPTY)
			type = CLOSE;
		right = r;
	...
	// If interval is empty set the interval type to CLOSE
	template<FloatLike IT> inline IT interval<IT>::leftinterval(const IT& l)
	{ 	
		if (type == EMPTY)
			type = CLOSE;
		left = l;
	...
	template<FloatLike IT> inline enum interval_type interval<IT>::intervaltype() const 
	{ return type; }
	...
	//
	template<FloatLike IT> inline enum interval_type interval<IT>::intervaltype(const enum interval_type to)
	{
		interval<IT> x = *this;
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
		...
		if (x.type != to)
		{
			...
			case CLOSE: 
				...
				if (to == LEFT_OPEN || to == OPEN)
					x.left = utils::nextafter(x.left, -infi);
				if (to == RIGHT_OPEN || to == OPEN)
					x.right = utils::nextafter(x.right, +infi);
				break;
			case OPEN: 
	...
	template<FloatLike IT> inline enum interval_decoration interval<IT>::intervaldecoration() const
	{
		return decoration;
	}

	// Set and  Return the interval decoration
	template<FloatLike IT> inline enum interval_decoration interval<IT>::intervaldecoration(const enum interval_decoration to)
	{	
		decoration = to;
		if(to==COMPUTE)
	...
	template<FloatLike IT> inline IT interval<IT>::inf(bool toclose) const
	{
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
		...
		if (type == LEFT_OPEN || type == OPEN) 
			adjustedLeft = utils::nextafter(left, (left <= right) ? +infi : -infi);

		// Adjust right boundary for open intervals, taking into account improper intervals
		if (type == RIGHT_OPEN || type == OPEN) 
			adjustedRight = utils::nextafter(right, (left <= right) ? -infi: +infi);
	...
	template<FloatLike IT> inline IT interval<IT>::sup(bool toclose) const 
	{
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
		if (isEmpty())	// If empty return -infinity
			return -infi;
	...
	template<FloatLike IT> inline IT interval<IT>::mid() const { 
	...
	template<FloatLike IT> inline IT interval<IT>::rad() const { 
		if (isEmpty()) return utils::get_nan(left);
	...
	template<FloatLike IT> inline IT interval<IT>::wid() const {
		if (isEmpty()) return utils::get_nan(left);
		IT r(right - left);
		if (r < IT(0)) r = -r; 
		return r;
	...
	template<FloatLike IT> inline IT interval<IT>::mig() const {
		if (isEmpty()) return utils::get_nan(left);
	...
	template<FloatLike IT> inline IT interval<IT>::mag() const {
		if (isEmpty()) return utils::get_nan(left);
		const IT l(utils::abs(inf()));
		const IT r(utils::abs(sup()));
		return std::max(l, r);
	...
	// Required is... methods
	template<FloatLike IT> inline bool interval<IT>::isProper() const { return left<=right; }
	template<FloatLike IT> inline bool interval<IT>::isImproper() const { return right<left; }
	template<FloatLike IT> inline bool interval<IT>::isPoint() const { return left==right; }
	template<FloatLike IT> inline bool interval<IT>::isEmpty() const { return type==EMPTY; }
	template<FloatLike IT> inline bool interval<IT>::isEntire() const
	{
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
	...
	template<FloatLike IT> inline enum int_class interval<IT>::isClass() const 
	{
	...
	template<FloatLike IT> inline bool interval<IT>::in(const IT& p) { return inf() <= p && p <= sup(); }


	// Return the interval as a String representation
	template<FloatLike IT> inline std::string interval<IT>::toString() const
	{
		std::string s;
	...
	//
	template<FloatLike IT> inline interval<IT>& interval<IT>::operator=( const interval<IT>& rhs )
	{
		left = rhs.left;
		right = rhs.right;
		type = rhs.type;
	...
	template<FloatLike IT> inline interval<IT>& interval<IT>::operator+=( const interval<IT>& rhs )
	{
		...
		if (xleft.second < IT(0))
			xleft.first=utils::nextafter(xleft.first, -infi);
		if (xright.second > IT(0))
			xright.first = utils::nextafter(xright.first, +infi);
		left = xleft.first;
		right = xright.first;
	#ifdef PHASE5
	...
	template<FloatLike IT> inline interval<IT>& interval<IT>::operator*=( const interval<IT>& rhs )
	{
		...
		auto multiply = [&](const IT& x, const IT& y) 
		{
			...
			const IT infi(infinity_interval<IT>());// infi(INFINITY);
			if (tmp.second < c0)
				res.left = utils::nextafter(tmp.first, -infi);
			if (tmp.second > c0)
				res.right = utils::nextafter(tmp.first, +infi);
			return res;
		};
	...
	template<FloatLike IT> inline interval<IT>& interval<IT>::operator/=(const interval<IT>& rhs)
	{
		...
		auto inverse = [&](const IT& y, const bool up)
		{
			...
			if (up == false){if (r < IT(0)) res = utils::nextafter(res, -infi);}
			else {if (r > IT(0)) res = utils::nextafter(res, +infi); };
	...
	//
	template<FloatLike IT> inline interval<IT> operator+(const interval<IT>& a, const interval<IT>& b)
	{
	...
	// Works for all classes
	//
	template<FloatLike IT> inline interval<IT> operator+(const interval<IT>& a)
	{
		return a;
	}
	...
	// Works for all classes
	//
	template<FloatLike IT> inline bool operator==(const interval<IT>& a, const interval<IT>& b)
	{
	...
	//
	template<FloatLike IT, FloatLike _X> inline bool operator==(const interval<IT>& a, const _X& b)
	{
		interval<IT> c(b);
		return a == c;
		}
	...
	// Works for all mixed classes
	//
	template<FloatLike IT, FloatLike _X> inline bool operator==(const _X& a, const interval<IT>& b)
	{
		interval<IT> c(a);
		return c == b;
		}
	...
	// Works for all classes
	//
	template<FloatLike IT, FloatLike _X> inline bool operator==(const interval<IT>& a, const interval<_X>& b)
	{
		interval<IT> c(b);
		return a == c;
	}
	...
	template<FloatLike IT> inline interval<IT> abs( const interval<IT>& a )
		{
	...
	template<FloatLike IT> inline IT intervaldistance(const interval<IT>& a, const interval<IT>& b)
		{
	...
	//
	template<FloatLike IT> inline std::pair<interval<IT>, interval<IT> > join(const interval<IT>& a, const interval<IT>& b)
	{
	...
	template<FloatLike IT> inline interval<IT> intersection(const interval<IT>& a, const interval<IT>& b)
	{
		interval<IT> c(a);
		c &= b;
		return c;
	...
	template<FloatLike IT> inline bool subset(const interval<IT>& a, const interval<IT>& b)
	{
	...
	template<FloatLike IT> inline bool interior(const interval<IT>& a, const interval<IT>& b)
	{
		if (b.inf() < a.inf() && a.sup() < b.sup())
			return true;
		return false;
	...
	template<FloatLike IT> inline bool precedes(const interval<IT>& a, const interval<IT>& b)
	{
		if (a.sup() < b.inf() )
			return true;
		return false;
	...
	// otherwise return 0
	template<FloatLike IT> inline int inclusion(const interval<IT>& a, const interval<IT>& b)
	{
		if( subset(a, b))
			return -1;
		if (subset(b, a))
	...
	template<FloatLike IT> inline bool empty(const interval<IT>& a) { return a.isEmpty(); }

	// empty(). The function version of the method .isEmpty()
	template<FloatLike IT> inline bool entire(const interval<IT>& a) { return a.isEntire();}
	...
	// sqr(x)=x^2
	template<FloatLike IT> inline interval<IT> sqr(const interval<IT>& x)
	{	
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
		const IT unfl(underflow_interval<IT>());
	...
	//
	template<FloatLike IT> inline interval<IT> sqrt(const interval<IT>& x)
	{	
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
		...
		if (utils::isinf(left) && utils::isinf(x.inf()))
			r = 0;	// When both is infinity
		if (r < IT(0) )
			left=utils::nextafter(left, -infi);
		...
		if (r > IT(0) )
			right=utils::nextafter(right, +infi);
	...
	template<FloatLike IT> inline interval<IT> floor(const interval<IT>& x)
	{
	...
	// ceil(x)
	template<FloatLike IT> inline interval<IT> ceil(const interval<IT>& x)
	{
		const IT left(x.inf(true));
		const IT right(x.sup(true));
	...
	//
	template<FloatLike IT> inline interval<IT> sgn(const interval<IT>& x)
	{
		const IT left(x.inf(true));
		const IT right(x.sup(true));
		IT left_min(IT(+1));
	...
	// log(x)
	template<FloatLike IT> inline interval<IT> log(const interval<IT>& x)
	{
		if (x.isEmpty() )
			return interval<IT>();	// Return the EMPTY interval;
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
		...
		else lower = utils::nextafter(lower, -infi); // Adjust for precision if not a shortcut value
		...
		else upper = utils::nextafter(upper, +infi); // Adjust for precision if not a shortcut value
	...
	// log10(x)
	template<FloatLike IT> inline interval<IT> log10(const interval<IT>& x)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
	...
	// exp(x)
	template<FloatLike IT> inline interval<IT> exp(const interval<IT>& x)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
	...
	// 
	template<FloatLike IT> inline interval<IT> pow(const interval<IT>& x, const IT y)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
		...
		*/
		...
		if (utils::floor(l) != l || utils::floor(r) != r)
	...
	// x
	// 
	template<FloatLike IT> inline interval<IT> pow(const interval<IT>& x, const interval<IT>& y)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
		interval<IT> c;
		...
		// if y is an integer?
		if (utils::floor(y.inf()) == y.inf() && utils::floor(y.sup()) == y.sup())
	...
	template<FloatLike IT> inline interval<IT> sin(const interval<IT>& x) 
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
		const IT pi = [&]()->IT { return utils::acos(IT(-1));	// More precise pi value
		...
		if (sin_min != IT(-1)  && sin_min != IT(0) )
			sin_min = utils::nextafter(sin_min, -infi);
		if( sin_max != IT(1) && sin_max != IT(0) )
			sin_max= utils::nextafter(sin_max, +infi);
	...
	// local float_precision declaration is performed at the precision of x. Implemented via the use of constexpr (requires c++17)
	// lambda functions. the variable l and r inherits the precision fromthe call to fmod()
	template<FloatLike IT> inline interval<IT> cos(const interval<IT>& x) 
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
		...
		if (cos_min != IT(-1) && cos_min != IT(0) )
			cos_min = utils::utils::nextafter(cos_min, -infi);
		if (cos_max != IT(1) && cos_max != IT(0) )
			cos_max = utils::utils::nextafter(cos_max, +infi);
	...
	// local float_precision declaration is performed at the precision of x. Implemented via the use of constexpr (requires c++17)
	// lambda functions. the variable l and r inherits the precision fromthe call to fmod()
	template<FloatLike IT> inline interval<IT> tan(const interval<IT>& x)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
		...
		// Established a safety interval around the result to ensure correct bound for the computation
		if (tan_min != pi)
			tan_min = utils::utils::nextafter(tan_min, -infi);
		if (tan_max != pi)
			tan_max = utils::utils::nextafter(tan_max, +infi);
	...
	// lambda functions. 
	//
	template<FloatLike IT> inline interval<IT> asin(const interval<IT>& x)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
	...
	//
	template<FloatLike IT> inline interval<IT> acos(const interval<IT>& x)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
	...
	// local float_precision declaration is performed at the precision of x. 
	//
	template<FloatLike IT> inline interval<IT> atan(const interval<IT>& x)
	{
	...
	template<FloatLike IT> inline interval<IT> sinh(const interval<IT>& x)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
	...
	template<FloatLike IT> inline interval<IT> cosh(const interval<IT>& x)
	{
		if (x.isEmpty()) return interval<IT>();	// Return the EMPTY interval;
	...
	template<FloatLike IT> inline interval<IT> tanh(const interval<IT>& x)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
	...
	template<FloatLike IT> inline interval<IT> asinh(const interval<IT>& x) 
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
		const interval<IT> one(1);
	...
	template<FloatLike IT> inline interval<IT> acosh(const interval<IT>& x)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
		const interval<IT> one(1);
	...
	template<FloatLike IT> inline interval<IT> atanh(const interval<IT>&x)
	{
		if (x.isEmpty())
			return interval<IT>();	// Return the EMPTY interval;
	...
	template<FloatLike IT> inline interval<IT> sinsimpel(const interval<IT>& x)
	{
		const IT infi(infinity_interval<IT>());// infi(INFINITY);
	...
	template<typename IT> constexpr interval<IT> pi_interval(const size_t precision)
	{
		if constexpr (std::is_same<IT, float>::value)
			...
			static_assert(isFloatLike<IT>::value, "Unsupported type for pi_interval.Type must be float, double, long double or float_precision.");
	}
	...
	template<typename IT> constexpr interval<IT> e_interval(const size_t precision)
	{
		if constexpr (std::is_same<IT, float>::value)
			...
			static_assert(std::is_floating_point<IT>::value, "Unsupported type for pi_interval.Type must be float, double, long double or float_precision.");
	}
	...
	template<typename IT> constexpr interval<IT> ln10_interval(const size_t precision)
	{
		if constexpr (std::is_same<IT, float>::value)
			...
			static_assert(isFloatLike<IT>::valu, "Unsupported type for pi_interval.Type must be float, double, long double or float_precision.");
	}
