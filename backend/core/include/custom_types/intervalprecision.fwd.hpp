...
namespace intprec{
    ...
    public:
		...
		template<typename _X> explicit interval(const _X&); // Singleton interval
		// Regular interval with interval_type (default CLOSE)
		template<typename _X, typename _Y> explicit interval(const _X& , const _Y&, const enum interval_type t=CLOSE);
		// constructor for any other type to IT. Both up and down conversions are possible
		// constructor for an interval<_X> argument
		template<typename _X> explicit interval(const interval<_X>& a); 
    ...
    template<FloatLike IT> interval<IT> operator+( const interval<IT>&, const interval<IT>& ); 
    template<FloatLike IT> interval<IT> operator+( const interval<IT>& );	// Unary
    template<FloatLike IT> interval<IT> operator-( const interval<IT>&, const interval<IT>& );
    template<FloatLike IT> interval<IT> operator-( const interval<IT>& );	// Unary 
    template<FloatLike IT> interval<IT> operator*(const interval<IT>&, const interval<IT>&);
    template<FloatLike IT> interval<IT> operator/( const interval<IT>&, const interval<IT>& );
    ...
    template<FloatLike IT> bool operator==(const interval<IT>&, const interval<IT>&);
    template<FloatLike IT> bool operator!=(const interval<IT>&, const interval<IT>&);
    template<FloatLike IT> bool operator>=(const interval<IT>&, const interval<IT>&);
    template<FloatLike IT> bool operator<=(const interval<IT>&, const interval<IT>&);
    template<FloatLike IT> bool operator>(const interval<IT>&, const interval<IT>&);
    ...
    // Other functions
    template<FloatLike IT> interval<IT> abs(const interval<IT>&);

    // Elementary functions
    template<FloatLike IT> interval<IT> sqr(const interval<IT>&);	// x^2
    template<FloatLike IT> interval<IT> sqrt(const interval<IT>&);	// sqrt(x)
    ...
    template <FloatLike IT, FloatLike _X> bool operator==(const interval<IT>&, const _X&);
    template <FloatLike IT, FloatLike _X> bool operator==(const _X&, const interval<IT>&);
    template <FloatLike IT, FloatLike _X> bool operator==(const interval<IT>&, const interval<_X>&);
    template <FloatLike IT, FloatLike _X> bool operator!=(const interval<IT>&, const _X&);
    template <FloatLike IT, FloatLike _X> bool operator!=(const _X&, const interval<IT>&);
    template <FloatLike IT, FloatLike _X> bool operator!=(const interval<IT>&, const interval<_X>&);
    template <FloatLike IT, FloatLike _X> bool operator>=(const interval<IT>&, const _X&);
    ...
    template<FloatLike IT> bool subset(const interval<IT>&, const interval<IT>&);
    template<FloatLike IT> bool interior(const interval<IT>&, const interval<IT>&);
    template<FloatLike IT> bool precedes(const interval<IT>&, const interval<IT>&);
    template<FloatLike IT> int inclusion(const interval<IT>&, const interval<IT>&);
    template<FloatLike IT> std::pair<interval<IT>, interval<IT> > join(const interval<IT>&, const interval<IT>&);
    template<FloatLike IT> interval<IT> intersection(const interval<IT>&, const interval<IT>&);

    template<FloatLike IT> bool empty(const interval<IT>&);
    template<FloatLike IT> bool entire(const interval<IT>&);
	...
	template<typename IT> constexpr interval<IT> pi_interval(const size_t precision = 0);
	// Get e at the precision for IT.(float_precision also based on the precision)
	template<typename IT> constexpr interval<IT> e_interval(const size_t precision = 0);
	// Get ln2 at the precision for IT.(float_precision also based on the precision)
	template<typename IT> constexpr interval<IT> ln2_interval(const size_t precision = 0);
	// Get ln10 at the precision for IT.(float_precision also based on the precision)
	template<typename IT> constexpr interval<IT> ln10_interval(const size_t precision = 0);

    // Inifinity declaration for the various types
    template<FloatLike T> T infinity_interval();

    // Underflow declaration for the various types
    template<FloatLike T> T underflow_interval();

    template<FloatLike IT> interval<IT> floor(const interval<IT>&);	// floor(x)
    template<FloatLike IT> interval<IT> ceil(const interval<IT>&);	// ceil(x)
    template<FloatLike IT> interval<IT> sgn(const interval<IT>&);	// sqn(x)
    template<FloatLike IT> interval<IT> log(const interval<IT>&);	// log(x)
    template<FloatLike IT> interval<IT> log10(const interval<IT>&);	// log10(x)
    template<FloatLike IT> interval<IT> exp(const interval<IT>&);	// exp(x)
    template<FloatLike IT> interval<IT> pow(const interval<IT>&, const interval<IT>&);	// pow(x,y)==x^y

    // Trigonometric functions
    template<FloatLike IT> interval<IT> sin(const interval<IT>&);	// sin(x)
    template<FloatLike IT> interval<IT> cos(const interval<IT>&);	// cos(x)
    template<FloatLike IT> interval<IT> tan(const interval<IT>&);	// tan(x)
    template<FloatLike IT> interval<IT> asin(const interval<IT>&);	// asin(x)
    template<FloatLike IT> interval<IT> acos(const interval<IT>&);	// acos(x)
    template<FloatLike IT> interval<IT> atan(const interval<IT>&);	// atan(x)

    // Hyperbolic functions
    template<FloatLike IT> interval<IT> sinh(const interval<IT>&);	// sin(x)
    template<FloatLike IT> interval<IT> cosh(const interval<IT>&);	// cos(x)
    template<FloatLike IT> interval<IT> tanh(const interval<IT>&);	// tan(x)
    template<FloatLike IT> interval<IT> asinh(const interval<IT>&);	// asin(x)
    template<FloatLike IT> interval<IT> acosh(const interval<IT>&);	// acos(x)
    template<FloatLike IT> interval<IT> atanh(const interval<IT>&);	// atan(x)

    template<FloatLike IT> IT intervaldistance(const interval<IT>&, const interval<IT>&);
    enum interval_type compute_interval_type(const enum interval_type, const enum interval_type);

    template<class _Ty> std::ostream& operator<<(std::ostream& strm, const interval<_Ty>& a);
    template<class _Ty> std::istream& operator>>( std::istream& strm, interval<_Ty>& c );
}

namespace Eigen {

    template<> struct NumTraits<intprec::interval<float>>
    ...
    template<> struct NumTraits<intprec::interval<double>>
...
//
    ...
    template<> struct NumTraits<intprec::interval<long double>>
    ...
    template<> struct NumTraits<intprec::interval<mpfr::mpreal>>
     : NumTraits<double> // permits to get the epsilon, dummy_precision, lowest, highest functions
    {
