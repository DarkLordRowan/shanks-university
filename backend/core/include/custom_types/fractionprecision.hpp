 ...
 * 01.04	HVE/20-Nov-2022	Adding long long and unsigned long long casting operator, added iszero() and isone()
 * method 01.05	HVE/27-Mar-2023	Added abs both as a mthod and as a frunction for fraction_precision
 *
...
[[maybe_unused]] static char _VFP_[] = "@(#)fractionprecision.h 01.05 -- Copyright (C) Henrik Vestermark";
...
// Notice construction,assignments always guarantees that normalized fraction sign is always in the numerator. As a
// consequence the sign method just return the sign of the numerator
template <class _Ty>
class fraction_precision {
    _Ty n, d;
...
public:
    ...
    // constructor
    fraction_precision(const _Ty& a = _Ty(0), const _Ty& b = _Ty(1)) : n(a), d(b) {
        normalize();
    }  // fraction constructions
    fraction_precision(const _Ty& whole, const _Ty& a, const _Ty& b) : n(a + whole * b), d(b) {
        normalize();
    }  // mixed number constructions
    // constructor for any other type to _Ty
    template <class _X>
    fraction_precision(const fraction_precision<_X>& a) : n(_Ty(a.numerator())), d(_Ty(a.denominator())) {}
    ...
    _Ty numerator(const fraction_precision<_Ty>& a) {
        return (n = a.numerator());
    }  // Set numerator from another fraction
    _Ty denominator(const _Ty& b) { return (d = b); }  // Set denominator
    _Ty denominator(const fraction_precision<_Ty>& b) {
        return (d = b.denominator());
    }  // Set denominator from another fraction
    ...
    _Ty reduce() const;   // Reduce the fraction by removing and returning the whole number from the fraction
                          // // Conversion methods. Safer and less ambiguous than overloading implicit/explicit
...
template <class _Ty>
std::ostream& operator<<(std::ostream& strm, const fraction_precision<_Ty>& a) {
    return strm << a.numerator() << "/" << a.denominator();
}

template <class _Ty>
std::istream& operator>>(std::istream& strm, fraction_precision<_Ty>& f) {
    _Ty n, d;
    char ch;
    strm >> n;
    ...
    if (!strm.fail()) f = fraction_precision<_Ty>(n, d);
    return strm;
}

// Arithmetic
template <class _Ty>
fraction_precision<_Ty> operator+(const fraction_precision<_Ty>&, const fraction_precision<_Ty>&);  // Binary
template <class _Ty>
fraction_precision<_Ty> operator+(const fraction_precision<_Ty>&);  // Unary
template <class _Ty>
fraction_precision<_Ty> operator-(const fraction_precision<_Ty>&, const fraction_precision<_Ty>&);  // Binary
template <class _Ty>
fraction_precision<_Ty> operator-(const fraction_precision<_Ty>&);  // Unary
...
fraction_precision<_Ty> operator/(const fraction_precision<_Ty>&, const fraction_precision<_Ty>&);  // Binary
template <class _Ty>
fraction_precision<_Ty> operator++(fraction_precision<_Ty>&);  // Prefix
template <class _Ty>
fraction_precision<_Ty> operator++(fraction_precision<_Ty>&, int);  // Postfix
template <class _Ty>
fraction_precision<_Ty> operator--(fraction_precision<_Ty>&);  // Prefix
template <class _Ty>
fraction_precision<_Ty> operator--(fraction_precision<_Ty>&, int);  // Postfix
...
template <class _Ty>
inline bool operator==(const fraction_precision<_Ty>&, const fraction_precision<_Ty>&);
template <class _Ty>
inline bool operator!=(const fraction_precision<_Ty>&, const fraction_precision<_Ty>&);
template <class _Ty>
inline bool operator>=(const fraction_precision<_Ty>&, const fraction_precision<_Ty>&);
...
//
template <class _Ty>
fraction_precision<_Ty>& fraction_precision<_Ty>::abs() {
    if (n < (_Ty)0) n = -n;
    if (d < (_Ty)0) d = -d;
...
//
template <class _Ty>
fraction_precision<_Ty>& fraction_precision<_Ty>::normalize() {
    _Ty z = gcd(n, d);
    if (z == (_Ty)0) throw divide_by_zero();
    n /= z;
    d /= z;
...
//
template <class _Ty>
fraction_precision<_Ty>& fraction_precision<_Ty>::inverse() {
    _Ty z;
    z = n;
    n = d;
    d = z;
...
//
template <class _Ty>
bool fraction_precision<_Ty>::iszero() const {  // Test for zero and return true or false
    if (n == (_Ty)0) return true;
    return false;
}
...
//
template <class _Ty>
bool fraction_precision<_Ty>::isone() const {  // Test for one and return true or false

    if (n == (_Ty)1 && d == (_Ty)1) return true;
    return false;
}
...
//
template <class _Ty>
_Ty fraction_precision<_Ty>::reduce() const {
    _Ty w = n / d;
    n %= d;
    return w;
}
...
//
template <class _Ty>
_Ty fraction_precision<_Ty>::whole() const {
...
//
template <class _Ty>
fraction_precision<_Ty>& fraction_precision<_Ty>::operator=(const fraction_precision<_Ty>& a) {
    n = a.numerator();
    d = a.denominator();
    normalize();
    return *this;
...
// lhs + rhs
template <class _Ty>
fraction_precision<_Ty> operator+(const fraction_precision<_Ty>& x, const fraction_precision<_Ty>& y) {
...
// +x
template <class _Ty>
fraction_precision<_Ty> operator+(const fraction_precision<_Ty>& x) {
    return x;
}
...
// Prefix ++
template <class _Ty>
fraction_precision<_Ty> operator++(fraction_precision<_Ty>& a) {
...
// Postfix ++
template <class _Ty>
fraction_precision<_Ty> operator++(fraction_precision<_Ty>& a, int) {
...
// lhs == rhs
template <class _Ty>
bool operator==(const fraction_precision<_Ty>& lhs, const fraction_precision<_Ty>& rhs) {
...
//
template <class _Ty>
inline fraction_precision<_Ty> abs(const fraction_precision<_Ty>& a) {
...
//
template <class _TY>
inline fraction_precision<_TY> gcd(const fraction_precision<_TY>& a) {
    return a.normalize();
}
