 ...
 *							Renamed _int_precision_umul to _int_precision_umul_school and
 * added _int_precision_umul as a common entry for multiplcation that branch out to the most optimal multiplication
 * algorithm to call. e.g. school, karatsuba, FFT or Schohage_strassen 03.09	HVE/21-Mar-2022 Added fixed size
 * arbitrry integers by introduce the mLimit field in the int_precision class. Added the method .precision() 03.10
 *	HVE/25-Mar-2022	Fixed a bug in the _int_precision_unegate() 03.11	HVE/7-Aug-2022	Handle a carry bug in
 * the _int_precision_umul_add() 03.12	HVE/24-Aug-2022	Cleaning up code 03.13	HVE/26-Aug-2022	Added int_precision
 * constructor for float and double for completeness 03.14	HVE/5-Sep-2022	Restore the _int_precision_compare() to
 * the previous pointer arguments 03.15	HVE/12-Dec-2022	Added declaration of factorial(), fallingfactorial(), binomial()
 * 03.16	HVE/25-Mar-2023	Added declaration of Stirling number of the first, second and third kind (last also
 * known as Lah number) 03.17	HVE/4-May-2023	Added the declaration of the jacobi symbol function 03.18
 *	HVE/10-May-2023	Added the declaration of miller_rabbin and baillie_PSW primality tester function 03.19
 *	HVE/24-May-2023	Reintroduce the fast division for integer /, %. 03.20	HVE/10-Jun-2023	Added the
 * random_precision class for arbitrary precision PRNG 03.21	HVE/19-Jun-2023	Added the following PRNGs as classes:
 * chacha20, xoshiro256pp, xoshiro256ss, xoshiro512pp, xoshiro512ss 03.22	HVE/01-Aug-2023	Code change to make the
 * source compiled under gcc 03.23	HVE/04-Aug-2023	added the fibonacci function 03.24	HVE/18-Aug-2024 Fixed an
 * issue reported by C++17 standard, that bool operator==(const random_precision<PrngType, ReturnType>& rhs) in
 * random_precision was missing the template argument. Same for bool operator!=(const random_precision<PrngType,
 * ReturnType>& rhs) 03.25	HVE/27-Nov-2024	Added istringsteam support for int_precision cin/cout >> operator
 *
...
[[maybe_unused]] static char _VI_[] = "@(#)iprecision.h 03.25 -- Copyright (C) Henrik Vestermark";
...
typedef std::uintmax_t iptype;  // The default size of the internal binary vector type, an unsigned 64bit. It should
                                // ALWAYS be set to the 'biggest' integer type. However performance will suffer if
                                // iptype < 64bit or not set to the maxium the enviroment can handle.
const unsigned int Bitsiptype =
    sizeof(iptype) * 8;  // Const use throughout the source which is the number of bits the iptype can hold.
...
template <typename T>
struct is_int_prec_supported : public std::false_type {};

template <std::integral T>
struct is_int_prec_supported<T> : public std::true_type {};

template <std::floating_point T>
struct is_int_prec_supported<T> : public std::true_type {};

template <typename T>
concept int_prec_supported = is_int_prec_supported<T>::value;
...
// for short we will use the notation that the least significant part of the number is stored in mNumber[0] and will be
// denoted a0, the most significant of the number in mNumber[n-1] as an-1 the radix, R will be 2^64 for iptype=uint64_t
// and 2^32 for iptype=uint3_t etc. the number in mNumber can be written as:
//		mNumber=a0*R^0+a1*R^1+a2*R^2...an-1*R^n-1
//
class int_precision {
    int mSign;  // Sign of the int_precision. Version 2+ only. In version 2 sign has been separated from mNumber to
                // avoid many uncessary copies and string.substr() calls mSign is either +1 or -1. For mNumber==0 then
                // sign is always +1
    size_t mLimit;  // By default int_precision i ulimited precision but it can be limit to force a certain size of an
                    // integer. e.g. 128bit has limit=2, 512bit has limit=8 etc. unlimit preciion has a limit on
                    // UINTMAX_MAX or SIZE_MAX
    std::vector<iptype>
        mNumber;  // The binary vector of iptype that holds the integer. Per definition the vector when the constructor
                  // is invoked will always be initialized to zero if no argument is provided.
...
public:
    ...
    int_precision(unsigned long long,
                  const size_t = SIZE_MAX);  // When initialized through an unsigned long. Same as uintmax_t
    ...
    int_precision(const std::vector<iptype>&,
                  const size_t = SIZE_MAX);  // When initialized through a std::vector<iptype>. Notice sign will be 1
                                             // since vector<iptype> is unsigned
    ...
    size_t precision()
        const;  // Return the maximum fixed integer precision the variable can hold in number of iptype units
    ...
    template <typename T>
    operator T() const {
        return static_cast<T>(mNumber[0] * mSign)
    };
#else
    operator long() const;
    operator int() const;
...
// Arithmetic
template <int_prec_supported _Ty>
inline int_precision operator+(const int_precision&, const _Ty&);
template <int_prec_supported _Ty>
...
inline int_precision operator<<(const int_precision&, const _Ty&);
inline int_precision operator<<(const int_precision&, const int_precision&);
// template <class _Ty> inline int_precision operator<<( const _Ty&, const int_precision& );  // Dont allow to avoid
// overloading conflict in streams library??
template <int_prec_supported _Ty>
inline int_precision operator>>(const int_precision&, const _Ty&);
template <int_prec_supported _Ty>
inline int_precision operator>>(const _Ty&, const int_precision&);
...
template <int_prec_supported _Ty>
inline bool operator==(const int_precision&, const _Ty&);
template <int_prec_supported _Ty>
...
template <class _TY>
inline _TY gcd(const _TY, const _TY);  // gcd template function. Use slow Euclid algorithm
...
extern int_precision stirling_first(const int_precision& n, const int_precision& k,
                                    const bool = false);                           // Stirling number of the first kind
extern int_precision stirling_second(const int_precision&, const int_precision&);  // Stirling number of the second kind
extern int_precision stirling_third(const int_precision&, const int_precision&,
                                    const bool = false);  // Stirling number of the third kind or Lah number
...
template <class _Ty, class _type = uint64_t>
class random_precision;                                // random precision class
...
template <class _TY>
inline std::vector<_TY> _precision_umul64(const _TY, const _TY);
...
inline int_precision::int_precision(const int_precision& s, const size_t limit) {
    ...
    mNumber = s.mNumber;
    if (mNumber.size() >
        mLimit)  // don't do mNumber.resize(mLimit) directly size it will realloacte the vector to an unrealistic size
...
inline int_precision::int_precision(const char* str, const size_t limit) {
    ...
    if (mNumber.size() > mLimit) mNumber.resize(mLimit);
}
...
//
inline int_precision::int_precision(float d, const size_t limit) {  // Call the constructor for double
...
//
inline int_precision::operator long long() const {  // Conversion to long long
...
//  loss in precision is possible
//
inline int_precision::operator long() const {  // Conversion to long
...
//  loss in precision is possible
//
inline int_precision::operator int() const {  // Conversion to int
...
//  loss in precision is possible
//
inline int_precision::operator short() const {  // Conversion to short
...
inline int_precision::operator signed char() const {  // Conversion to signed char
    return static_cast<signed char>(mNumber[0] * mSign);
}
...
//  loss in precision is possible
//
inline int_precision::operator unsigned long long() const {  // Conversion to unsigned long long
...
//  loss in precision is possible
//
inline int_precision::operator unsigned long() const {  // Conversion to unsigned long
...
//  loss in precision is possible
//
inline int_precision::operator unsigned int() const {  // Conversion to unsigned int
...
//  loss in precision is possible
//
inline int_precision::operator unsigned short() const {  // Conversion to unsigned short
...
//  loss in precision is possible
//
inline int_precision::operator unsigned char() const {  // Conversion to char
...
//
inline int_precision::operator double() const {  // Conversion to double
...
//
inline int_precision::operator float() const {  // Conversion to float
...
//
inline int int_precision::change_sign() {
    mSign *= -1;
    return mSign;
}  // Toggle and return sign
...
inline size_t int_precision::precision(const size_t p) {
    ...
    if (mLimit < mNumber.size()) mNumber.resize(mLimit);
...
//
inline int_precision& int_precision::abs() {
    mSign = 1;
    return *this;
}  // Change sign to + and return number
...
inline void int_precision::setbit(size_t i) {
    ...
    if (n >= mNumber.size()) mNumber.insert(mNumber.end(), n + 1 - mNumber.size(), 0);
...
inline int_precision& int_precision::operator+=(const int_precision& rhs) {
    ...
    if (rhs.mSign == mSign)
        mNumber =
            _int_precision_uadd(const_cast<std::vector<iptype>&>(rhs.mNumber), mNumber);  // Add and no change of sign
    else {
        ...
        {
            mSign = rhs.mSign;
            mNumber = _int_precision_usub(&wrap_flag, const_cast<std::vector<iptype>&>(rhs.mNumber),
                                          mNumber);  // Subtract and change to sign1
        } else if (compare < 0)
            mNumber = _int_precision_usub(
                &wrap_flag, mNumber, const_cast<std::vector<iptype>&>(rhs.mNumber));  // Subtract and no change in sign
        else {                                                                        // result is 0
...
inline int_precision& int_precision::operator/=(const int_precision& rhs) {
    ...
    // do faster floating point division if denominator is greater than 2^32 otherwise a udiv_short is 1'000 times
    // faster
    ...
    else {  // Check for division of of number that can safely be done using 64bit binary division
...
inline int_precision& int_precision::operator%=(const int_precision& rhs) {
    ...
    if (rhs.mNumber.size() == 1 && (rhs.mNumber.front() >> 32) == 0)   // Make short rem
        ...
        mNumber = _int_precision_urem(
            mNumber, const_cast<std::vector<iptype>&>(rhs.mNumber));  // regular rem. sign stay the same
...
//
template <int_prec_supported _Ty>
inline int_precision operator+(const int_precision& lhs, const _Ty& rhs) {
...
inline int_precision operator+(const int_precision& lhs, const int_precision& rhs) { return int_precision(lhs) += rhs; }
...
//  Do nothing
//
inline int_precision operator+(const int_precision& a) { return a; }
...
inline bool operator==(const int_precision& a, const int_precision& b) {
    if (a.sign() == b.sign() &&
        _int_precision_compare(const_cast<int_precision&>(a).pointer(), const_cast<int_precision&>(b).pointer()) ==
            0)  // Same return true
        return true;
    return false;
}
inline bool operator!=(const int_precision& a, const int_precision& b) { return a == b ? false : true; }

inline bool operator>(const int_precision& a, const int_precision& b) { return b < a && a != b ? true : false; }
inline bool operator<(const int_precision& a, const int_precision& b) {
    int sign1, sign2, cmp;
    ...
    if (sign1 == 1 && a.size() > b.size())  // Different therefore false
        return false;
    if (sign1 == -1 && a.size() > b.size()) return true;
    if (sign1 == -1 && a.size() < b.size()) return false;
...
//  1) if sign different then result is false. We actual don't do this test because of -0==+0 we should not ocuured but
//  just in case 2) if length is different then the result is false 3) use core compare to determine boolean value
//
template <int_prec_supported _Ty>
inline bool operator==(const int_precision& a, const _Ty& b) {
    int_precision c(b);
    if (a.sign() == c.sign() &&
...
inline bool operator==(const _Ty& a, const int_precision& b) {
    int_precision c(a);
    if (c.sign() == b.sign() &&
        _int_precision_compare(const_cast<int_precision&>(c).pointer(), const_cast<int_precision&>(b).pointer()) == 0)
        return true;
    return false;
}
...
//
template <int_prec_supported _Ty>
inline bool operator<(const int_precision& a, const _Ty& c) {
    int sign1, sign2, cmp;
    int_precision b(c);
...
//
template <class _Ty>
inline _Ty gcd(const _Ty lhs, const _Ty rhs) {
    _Ty tmp, a = lhs, b = rhs;
...
//  Negate Integer
//
inline bool operator!(const int_precision& a) { return a.iszero(); }
...
//  Negate Integer
//
inline bool operator&&(const int_precision& a, const int_precision& b) { return !a.iszero() && !b.iszero(); }
...
//
//
template <class PrngType, class ReturnType>
class random_precision {
    PrngType generator;
...
public:
    random_precision(const int_precision& val = std::random_device{}()) {  // Initialization
        seed(val);
    }

    random_precision(const std::seed_seq& seeds) {  // Initialization through seed_seq seed
        seed(seeds);
    }

    void seed(const int_precision& seed_value) {  // Seed value is either a uint32_t or uint64_t
    ...
    static int_precision min() { return int_precision(0ull); }

    static int_precision max(const std::uintmax_t bitcnt = 64) {
        if (bitcnt <= 64) return int_precision(~0ull);
        int_precision m;
    ...
    void discard(const unsigned long long z) {
        for (unsigned long long i = z; i > 0; --i) (void)operator()();
    }
};
...
public:
    ...
    void seed(const uint32_t s = std::random_device{}()) {
        ...
        key_.clear();
        for (int i = 0; i < 16; ++i) key_.push_back(static_cast<uint8_t>(dis(gen)));
        nonce_.clear();
        for (int i = 0; i < 8; ++i) nonce_.push_back(static_cast<uint8_t>(dis(gen)));
        counter_ = gen();
        position_ = 0;
    }
    ...
    {                                // Initialization through seed_seq seed
        ...
        key_.clear();
        for (int i = 0; i < 16; ++i) key_.push_back(static_cast<uint8_t>(sequencekey[i]));
        seeds.generate(sequencenonce.begin(), sequencenonce.end());
        nonce_.clear();
        for (int i = 0; i < 8; ++i) nonce_.push_back(static_cast<uint8_t>(sequencenonce[i]));
    ...
    // Return min value
    static constexpr uint32_t min() { return uint32_t(0ul); }
    // Return max value
...
public:
    xoshiro256pp(const result_type val = std::random_device{}() /* 5489u*/) {  // Initialization
        seed(val);
    }
    ...
    void seed(const result_type seed_value) {
        for (int i = 0; i < 4; ++i) s[i] = splitmix64(seed_value + i);
    }
    ...
    {                                // Initialization through seed_seq seed
        ...
        seeds.generate(sequence.begin(), sequence.end());
        for (int i = 0; i < 4; ++i) s[i] = splitmix64(static_cast<result_type>(sequence[i]));
    }

    static result_type min() { return result_type(0u); }

    static result_type max() { return std::numeric_limits<result_type>::max(); }

    result_type operator()() {  /// 256++
    ...
    bool operator==(const xoshiro256pp& rhs) const { return this->s == rhs.s; }
...
//   a 64-bit seed, we suggest to seed a splitmix64 generator and use its
//   output to fill s.
class xoshiro256ss {  // Private
    using result_type = uint64_t;
    std::array<result_type, 4> s;  // Internal state 256 bits

    static inline result_type rotl(const result_type x, const int k) { return (x << k) | (x >> (64 - k)); }
...
public:
    inline xoshiro256ss(const result_type val = std::random_device{}()) {  // Initialization
        seed(val);
    }
    ...
    {                                       // Initialization through seed_seq seed
        std::array<unsigned, 4> sequence;
        seeds.generate(sequence.begin(), sequence.end());
        for (size_t i = 0; i < sequence.size(); ++i) s[i] = splitmix64(static_cast<result_type>(sequence[i]));
    }

    static result_type min() { return result_type(0ull); }

    static result_type max() { return std::numeric_limits<result_type>::max(); }

    inline result_type operator()() {  // 256**
    ...
    inline bool operator==(const xoshiro256ss& rhs) const { return this->s == rhs.s; }
...
public:
    inline xoshiro512pp(const result_type val = std::random_device{}() /*5489u*/) {  // Initialization
        seed(val);
    }
    ...
    inline result_type operator()() {  // 512++
    ...
    inline bool operator==(const xoshiro512pp& rhs) const { return this->s == rhs.s; }
...
public:
    inline xoshiro512ss(const result_type val = std::random_device{}()) {  // Initialization
        seed(val);
    }
    ...
    inline void seed(const result_type seed_value) {  // Regular seed
        for (int i = 0; i < 8; ++i) {
            s[i] = splitmix64(seed_value + i);
        }
    }
    ...
    inline bool operator==(const xoshiro512ss& rhs) const { return this->s == rhs.s; }
