...
struct uniform_noise<shanks::profiling::OperationCounting<T>>{
    static shanks::profiling::OperationCounting<T> generate(const shanks::profiling::OperationCounting<T>& left, const shanks::profiling::OperationCounting<T>& right, std::mt19937_64& rng){
...
inline mpfr::mpreal uniform_noise<mpfr::mpreal>::generate(const mpfr::mpreal& left, const mpfr::mpreal& right, std::mt19937_64& rng){
...
struct uniform_noise<T>{
     static T generate(const typename GetUnderlyingType<T>::value left, const typename GetUnderlyingType<T>::value right, std::mt19937_64& rng);
};

template<typename T>
requires ComplexLike<T> || IntervalLike<T>
T uniform_noise<T>::generate(const typename GetUnderlyingType<T>::value left, const typename GetUnderlyingType<T>::value right, std::mt19937_64& rng){

    return utils::cast<T, typename GetUnderlyingType<T>::value>()(uniform_noise<typename GetUnderlyingType<T>::value>::generate(left, right, rng)); 
