...
T uniform_noise<T>::generate(const typename GetUnderlyingType<T>::value left,
    ...
    if constexpr (IntervalLike<NextT>) {
        return utils::cast<T, NextT>()(uniform_noise<NextT>::generate(left.mid(), right.mid(), rng));
    } else {
        return utils::cast<T, NextT>()(uniform_noise<NextT>::generate(utils::cast<BaseOfNextT, NextT>()(left),
