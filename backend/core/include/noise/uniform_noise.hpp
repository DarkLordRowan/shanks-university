...
T generate_uniform_noise(const T& inf, const T& sup, std::mt19937_64& rng) {
    ...
    if (inf.real() >= sup.real() || inf.imag() >= sup.imag()) throw std::invalid_argument("Invalid borders for uniform noise generation.");
...
T generate_uniform_noise(const T& inf, const T& sup, std::mt19937_64& rng) {
    ...
    if (inf.leftinterval() >= sup.rightinterval()) throw std::invalid_argument("Invalid borders for uniform noise generation.");
