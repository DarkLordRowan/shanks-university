...
T generate_poisson_noise(const T& lambda, std::mt19937_64& rng) {
    if (lambda.real() <= utils::cast<typename T::value_type>(0) || lambda.imag() <= utils::cast<typename T::value_type>(0))
...
T generate_poisson_noise(const T& lambda, std::mt19937_64& rng) {
    if (lambda.leftinterval() <= utils::cast<typename T::value_type>(0)) throw std::invalid_argument("Lambda must be positive for Poisson distribution.");
