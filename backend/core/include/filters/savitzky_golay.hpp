...
namespace shanks
{
    namespace filters
    {
         ...
         * @authors Naumov A.U., Lykov D.S., Kreynin R.G.
         * @param data (std::vector<Scalar>), window_length (size_t), polyorder (size_t), derive (size_t), delta
         * (Scalar)
        ...
        std::vector<Scalar> savitzky_golay_filter(const std::vector<Scalar>& data,
        ...
        )
        {
            ...
            if (window_length > data.size())
                throw std::invalid_argument("window's length is bigger than data's size");
            if (polyorder >= window_length)
                throw std::invalid_argument("polyorder is bigger or equal than window's length");
            ...
            Eigen::Vector<Scalar, Eigen::Dynamic> v =
                Eigen::Vector<Scalar, Eigen::Dynamic>::LinSpaced(window_length, -N, N);
            ...
            Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic> coeff_mat = (x.transpose() * x).inverse() *
                                                                              x.transpose();
            ...
            std::vector<Scalar> padded_vector(data.size() + (window_length - 1) * 2,
                                              utils::cast<Scalar>(0.0, precision));
