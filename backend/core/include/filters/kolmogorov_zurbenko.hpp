...
std::vector<Scalar> kolmogorov_zurbenko_filter(const std::vector<Scalar>& data, size_t window_length /*m*/,
                                               size_t degree /*k*/
) {
    ...
    std::vector<Scalar> zur_coeffs(size, utils::cast<Scalar>(0.0, precision));
    const size_t N_pad = size / 2;
    ...
    for (size_t i{0}; i < result.size(); ++i)
        for (size_t j{0}; j < size; ++j) result[i] += zur_coeffs[j] * padded_vector[i + j];
