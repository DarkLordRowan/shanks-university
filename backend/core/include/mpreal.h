...
#    define MPREAL_HAVE_DYNAMIC_STD_NUMERIC_LIMITS                                                                     \
        1 // Enable extended std::numeric_limits<mpfr::mpreal> specialization.
...
namespace mpfr
{
    ...
    public:
        ...
        {
            return (signed char)toLong();
        }
        explicit operator unsigned char() const
        {
            return (unsigned char)toULong();
        }
...
// MPFR < 2.4.2 Specifics
    ...
    inline bool operator>(const unsigned long int b, const mpreal& a)
    {
        return !isnan(a) && (mpfr_cmp_ui(a.mpfr_srcptr(), b) < 0);
    }
    inline bool operator>(const unsigned int b, const mpreal& a)
    {
        return !isnan(a) && (mpfr_cmp_ui(a.mpfr_srcptr(), b) < 0);
    }
    inline bool operator>(const long int b, const mpreal& a)
    {
        return !isnan(a) && (mpfr_cmp_si(a.mpfr_srcptr(), b) < 0);
    }
    inline bool operator>(const int b, const mpreal& a)
    {
        return !isnan(a) && (mpfr_cmp_si(a.mpfr_srcptr(), b) < 0);
    }
    inline bool operator>(const long double b, const mpreal& a)
    {
        return !isnan(a) && (b == b) && (mpfr_cmp_ld(a.mpfr_srcptr(), b) < 0);
    }
    inline bool operator>(const double b, const mpreal& a)
    {
        return !isnan(a) && (b == b) && (mpfr_cmp_d(a.mpfr_srcptr(), b) < 0);
    }
    inline bool operator>(const long long int b, const mpreal& a)
    {
        return !isnan(a) && mpreal(b) > a;
    }
    inline bool operator>(const unsigned long long int b, const mpreal& a)
    {
        return !isnan(a) && mpreal(b) > a;
    }
    ...
    //////////////////////////////////////////////////////////////////////////
    // Type Converters
    inline bool mpreal::toBool() const
    {
    ...
    inline ::mpfr_ptr mpreal::mpfr_ptr()
    {
        return mp;
    }
    inline ::mpfr_srcptr mpreal::mpfr_ptr() const
    {
        return mp;
    }
    inline ::mpfr_srcptr mpreal::mpfr_srcptr() const
    {
        return mp;
    }
...
#define MPREAL_BINARY_MATH_FUNCTION_UI_BODY(f, u)                                                                      \
    ...
    inline mp_exp_t ilogb(const mpreal& x)
    {
        return x.get_exp();
    }
    ...
    inline const mpreal log_ui(unsigned long int n,
                               mp_prec_t prec = mpreal::get_default_prec(),
    ...
    /* f(x,u) = f(2*pi*x/u) */
    inline const mpreal cosu(const mpreal& x, unsigned long u, mp_rnd_t r = mpreal::get_default_rnd())
    {
        MPREAL_BINARY_MATH_FUNCTION_UI_BODY(cosu, u);
    }
    inline const mpreal sinu(const mpreal& x, unsigned long u, mp_rnd_t r = mpreal::get_default_rnd())
    {
        MPREAL_BINARY_MATH_FUNCTION_UI_BODY(sinu, u);
    }
    inline const mpreal tanu(const mpreal& x, unsigned long u, mp_rnd_t r = mpreal::get_default_rnd())
    {
        MPREAL_BINARY_MATH_FUNCTION_UI_BODY(tanu, u);
    }
    inline const mpreal acosu(const mpreal& x, unsigned long u, mp_rnd_t r = mpreal::get_default_rnd())
    {
        MPREAL_BINARY_MATH_FUNCTION_UI_BODY(acosu, u);
    }
    inline const mpreal asinu(const mpreal& x, unsigned long u, mp_rnd_t r = mpreal::get_default_rnd())
    {
        MPREAL_BINARY_MATH_FUNCTION_UI_BODY(asinu, u);
    }
    inline const mpreal atanu(const mpreal& x, unsigned long u, mp_rnd_t r = mpreal::get_default_rnd())
    {
...
#    endif
    ...
    inline const mpreal pow(const unsigned int a, const long int b, mp_rnd_t rnd_mode)
    {
        if (b > 0)
            return pow(
                static_cast<unsigned long int>(a), static_cast<unsigned long int>(b), rnd_mode); // mpfr_ui_pow_ui
        else
...
namespace std
{
    ...
    public:
        ...
        inline static const mpfr::mpreal infinity()
        {
        ...
        inline static int digits(const mpfr::mpreal& x)
        {
