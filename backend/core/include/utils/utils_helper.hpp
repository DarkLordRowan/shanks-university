...
T utils::nextafter(const T& x, const T& y){
    if constexpr(std::is_same<T, float>::value) return std::nextafterf(x,y);
    else if constexpr(std::is_same<T, double>::value) return std::nextafter(x,y);
    else if constexpr(std::is_same<T, long double>::value) return std::nextafterl(x,y);
    #ifdef __MPREAL_H__
    else if constexpr (std::is_same<T, mpfr::mpreal>::value) return mpfr::nextafter(x,y);
	#endif
    else{
...
T utils::fmod(const T& x, const T& y){
    if constexpr(std::is_same<T, float>::value) return std::fmodf(x, y);
    else if constexpr(std::is_same<T, double>::value) return std::fmod(x, y);
    else if constexpr(std::is_same<T, long double>::value) return std::fmodl(x, y);
    #ifdef __MPREAL_H__
    else if constexpr (std::is_same<T, mpfr::mpreal>::value) return mpfr::fmod(x,y);
	#endif
    else{
        static_assert(dependent_false<T>::value, "utils::get_nan not implemented for this type");
    }
...
T utils::floor(const T& x){
    if constexpr(std::is_same<T, float>::value) return std::floorf(x);
    else if constexpr(std::is_same<T, double>::value) return std::floor(x);
    else if constexpr(std::is_same<T, long double>::value) return std::floorl(x);
    #ifdef __MPREAL_H__
    else if constexpr (std::is_same<T, mpfr::mpreal>::value) return mpfr::floor(x);
	#endif
    else{
        static_assert(dependent_false<T>::value, "utils::get_nan not implemented for this type");
    }
...
T utils::ceil(const T& x){
    if constexpr(std::is_same<T, float>::value) return std::ceilf(x);
    else if constexpr(std::is_same<T, double>::value) return std::ceil(x);
    else if constexpr(std::is_same<T, long double>::value) return std::ceill(x);
    #ifdef __MPREAL_H__
    else if constexpr (std::is_same<T, mpfr::mpreal>::value) return mpfr::ceil(x);
	#endif
    else{
        static_assert(dependent_false<T>::value, "utils::get_nan not implemented for this type");
    }
...
std::string utils::to_string(const T& x){ 
    ...
    else if constexpr (is_interval<T>::value) return x.toString();
    else {
...
T utils::get_nan(const T& x) {
    if constexpr(std::is_same<T, float>::value) return std::nanf("");
    else if constexpr(std::is_same<T, double>::value) return std::nan("");
    else if constexpr(std::is_same<T, long double>::value) return std::nanl("");
    #ifdef __MPREAL_H__
    else if constexpr (std::is_same<T, mpfr::mpreal>::value){
...
bool utils::isnan(const T& x){
    if constexpr (std::is_floating_point<T>::value || std::is_integral<T>::value) return std::isnan(x);
    else if constexpr (is_complex_t<T>::value || is_complex_custom<T>::value) return utils::isnan(x.real()) || utils::isnan(x.imag());
    #ifdef __MPREAL_H__
    else if constexpr (std::is_same<T, mpfr::mpreal>::value){
		return mpfr::isnan(x);
	}
...
bool utils::isinf(const T& x){
    if constexpr(is_standard_types<T>::value) return std::isinf(x);
    else if constexpr(is_complex_custom<T>::value) return utils::isinf(x.real()) || utils::isinf(x.imag());
    else if constexpr(is_interval<T>::value) return utils::isinf(x.leftinterval()) || utils::isinf(x.rightinterval());
    #ifdef __MPREAL_H__
    else if constexpr (std::is_same<T, mpfr::mpreal>::value) return mpfr::isinf(x);
	#endif
    else {
...
T utils::epsilon(const T& x){
	...
	#endif
    else if constexpr(is_interval<T>::value) return utils::cast<T>(utils::epsilon(x.leftinterval()));
    else {
