...
inline To utils::cast(const From& x, const std::size_t precision){
	...
	else if constexpr (isComplexLike<To>::value && !isComplexLike<From>::value) return std::complex(utils::cast<typename real_of<To>::value>(x));
	...
	// Cast to interval type
		...
		if constexpr (isFloatLike<From>::value) return static_cast<To>(x);
		...
		else if constexpr (is_interval<From>::value) return static_cast<To>(x);
	}
