...
class AccelRecord:
    ...
    dev_m: np.ndarray
    dev_e: np.ndarray
    # Mask for valid entries (since some might be None)
...
class DataLoader:
    ...
    def __init__(self, data_dir: str):
        ...
        Args:
            data_dir: The directory containing 'series' and 'accelerations' subdirectories.
        """
        self.data_dir = data_dir
        self.series_df = pl.scan_parquet(os.path.join(data_dir, "series"), hive_partitioning=True)
        self.accel_df = pl.scan_parquet(os.path.join(data_dir, "accelerations"), hive_partitioning=True)
    ...
    def _compute_metadata(self) -> Metadata:
        ...
        precisions = self.series_df.select("precision").unique().collect().get_column("precision").drop_nulls().sort().to_list()
        series_names = self.series_df.select("series_name").unique().collect().get_column("series_name").drop_nulls().sort().to_list()
        accel_names = self.accel_df.select("accel_name").unique().collect().get_column("accel_name").drop_nulls().sort().to_list()
        m_values = self.accel_df.select("m_value").unique().collect().get_column("m_value").drop_nulls().sort().to_list()
    ...
    def _get_unique_param_info(self, df: pl.LazyFrame, column: str) -> Dict[str, List[str]]:
        """
        Helper to extract unique values from nested struct fields.
        
        Args:
            df: The LazyFrame to query.
            column: The name of the struct column.
            
        Returns:
            A dictionary mapping field names to lists of unique values (as strings).
        """
        res = {}
        ...
        if isinstance(schema, pl.Struct):
            for field in schema.fields:
                ...
                unique_vals = df.select(pl.col(column).struct.field(field_name)).unique().collect().get_column(field_name).drop_nulls().sort().to_list()
                if unique_vals:
                    res[field_name] = [str(v) for v in unique_vals]
        return res

    def _parse_sci_expr(self, col_expr: pl.Expr, default_val: Optional[float] = None) -> Tuple[pl.Expr, pl.Expr]:
        """
        Creates a Polars expression to parse scientific notation strings.
        
        Args:
            col_expr: The string column expression to parse.
            default_val: Value to use if mantissa is null/missing.
            
        Returns:
            A tuple of (mantissa_expr, exponent_expr).
        """
    ...
    def filter_data(self, filters: Dict[str, Any]) -> List[Tuple[SeriesRecord, List[AccelRecord]]]:
        """
        Filters the dataset based on provided criteria and returns structured records.
        
        Args:
            filters: Dictionary of filter keys and allowed values.
            
        Returns:
            A list of tuples, each containing a SeriesRecord and its associated AccelRecords.
        """
        import time
        t0 = time.time()
        ...
        if filters.get('base_series'):
            s_df = s_df.filter(pl.col("series_name").is_in(list(filters['base_series'])))
        
        for param, values in filters.get('series_params', {}).items():
            if values:
                s_df = s_df.filter(pl.col("arguments").struct.field(param).is_in(list(values)))

        # Define expression to transform computed list
        # Input: List<Struct<n, value<real, imag>, deviation>> (strings)
        # Output: List<Struct<n, vr_m, vr_e, vi_m, vi_e, d_m, d_e>> (numbers)
        ...
        s_transformed = s_df.with_columns(transform_computed("computed", has_n=True).alias("computed_parsed"))
        ...
        t2 = time.time()
        print(f"[Timing] Series collect to arrow: {t2 - t1:.3f}s (rows: {series_table.num_rows})")
        
        if series_table.num_rows == 0:
            return []

        # Get series IDs to filter accelerations
        # series_id column in arrow table
        ...
        for param, values in filters.get('accel_params', {}).items():
            if values:
                a_df = a_df.filter(pl.col("additional_args").struct.field(param).is_in(list(values)))

        # Same transformation for accel computed
        a_transformed = a_df.with_columns(transform_computed("computed", has_n=False).alias("computed_parsed"))
        
        t3 = time.time()
        accel_table = a_transformed.collect().to_arrow()
        t4 = time.time()
        print(f"[Timing] Accel collect to arrow: {t4 - t3:.3f}s (rows: {accel_table.num_rows})")
        ...
        if accel_table.num_rows > 0:
            ...
            for batch in batches:
                ...
                for field_name in ["vr_m", "vr_e", "vi_m", "vi_e", "d_m", "d_e"]:
                     child_arrays[field_name] = child_struct.field(field_name).to_numpy(zero_copy_only=False)
                ...
                for i in range(batch.num_rows):
                    ...
                    args = {k: str(v) for k, v in args_scalar.as_py().items() if v is not None}
                    ...
                    errors = [ErrorInfo(n=int(e.get('n',0)), message=e.get('message','')) for e in (err_list or []) if e]
                    events = [EventInfo(n=int(e.get('n',0)), name=e.get('name',''), description=e.get('description','')) for e in (evt_list or []) if e]
        ...
        print(f"[Timing] Accel processing loop: {time.time() - t_start_proc_accel:.3f}s")
        ...
        for batch in batches:
            ...
            if has_n:
                child_arrays['n'] = child_struct.field("n").to_numpy(zero_copy_only=False)
            else:
            ...
            for i in range(batch.num_rows):
                ...
                series_rec = SeriesRecord(
                    ...
                    series_id=sid,
                    name=b_name[i].as_py(),
                    arguments={k: str(v) for k, v in b_args[i].as_py().items() if v is not None},
        ...
        print(f"[Timing] Series processing loop: {time.time() - t_start_proc_series:.3f}s")
        return result
